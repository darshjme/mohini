...
```bash
GROQ_API_KEY=<key> target/release/mohini.exe start &
...
## Architecture Notes
- **Don't touch `mohini-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `mohini.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
