# Contributing to Mohini

Thank you for your interest in contributing to Mohini. This guide covers everything you need to get started, from setting up your development environment to submitting pull requests.
...
```bash
cargo test -p mohini-kernel
cargo test -p mohini-runtime
cargo test -p mohini-memory
```
...
- **Naming**:
  - Types: `PascalCase` (e.g., `MohiniKernel`, `AgentManifest`)
  - Functions/methods: `snake_case`
  - Constants: `SCREAMING_SNAKE_CASE`
  - Crate names: `mohini-{name}` (kebab-case)
...
Mohini is organized as a Cargo workspace with 14 crates:

| Crate | Role |
|-------|------|
| `mohini-types` | Shared type definitions, taint tracking, manifest signing (Ed25519), model catalog, MCP/A2A config types |
| `mohini-memory` | SQLite-backed memory substrate with vector embeddings, usage tracking, canonical sessions, JSONL mirroring |
| `mohini-runtime` | Agent loop, 3 LLM drivers (Anthropic/Gemini/OpenAI-compat), 38 built-in tools, WASM sandbox, MCP client/server, A2A protocol |
| `mohini-hands` | Hands system (curated autonomous capability packages), 7 bundled hands |
| `mohini-extensions` | Integration registry (25 bundled MCP templates), AES-256-GCM credential vault, OAuth2 PKCE |
| `mohini-kernel` | Assembles all subsystems: workflow engine, RBAC auth, heartbeat monitor, cron scheduler, config hot-reload |
| `mohini-api` | REST/WS/SSE API (Axum 0.8), 76 endpoints, 14-page SPA dashboard, OpenAI-compatible `/v1/chat/completions` |
| `mohini-channels` | 40 channel adapters (Telegram, Discord, Slack, WhatsApp, and 36 more), formatter, rate limiter |
| `mohini-wire` | MMP (Mohini Protocol): TCP P2P networking with HMAC-SHA256 mutual authentication |
| `mohini-cli` | Clap CLI with daemon auto-detect (HTTP mode vs. in-process fallback), MCP server |
| `mohini-migrate` | Migration engine for importing from LegacyImport (and future frameworks) |
| `mohini-skills` | Skill system: 60 bundled skills, SkillHub marketplace, LegacyImport compatibility, prompt injection scanning |
| `mohini-desktop` | Tauri 2.0 native desktop app (WebView + system tray + single-instance + notifications) |
| `xtask` | Build automation tasks |

### Key Architectural Patterns

- **`KernelHandle` trait**: Defined in `mohini-runtime`, implemented on `MohiniKernel` in `mohini-kernel`. This avoids circular crate dependencies while enabling inter-agent tools.
...
```bash
mohini agent spawn agents/my-agent/agent.toml
```
...
Channel adapters live in `crates/mohini-channels/src/`. Each adapter implements the `ChannelAdapter` trait.

### Steps

1. Create a new file: `crates/mohini-channels/src/myplatform.rs`
...
3. Register the module in `crates/mohini-channels/src/lib.rs`:

```rust
pub mod myplatform;
```

4. Wire it up in the channel bridge (`crates/mohini-api/src/channel_bridge.rs`) so the daemon starts it alongside other adapters.

5. Add configuration support in `mohini-types` config structs (add a `[channels.myplatform]` section).

6. Add CLI setup wizard instructions in `crates/mohini-cli/src/main.rs` under `cmd_channel_setup`.
...
Built-in tools are defined in `crates/mohini-runtime/src/tool_runner.rs`.
