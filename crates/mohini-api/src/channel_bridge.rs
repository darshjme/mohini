//! Channel bridge wiring — connects the Mohini kernel to channel adapters.
//!
//! Implements `ChannelBridgeHandle` on `MohiniKernel` and provides the
//! `start_channel_bridge()` entry point called by the daemon.

use mohini_channels::bridge::{BridgeManager, ChannelBridgeHandle};
use mohini_channels::discord::DiscordAdapter;
use mohini_channels::email::EmailAdapter;
use mohini_channels::google_chat::GoogleChatAdapter;
use mohini_channels::irc::IrcAdapter;
use mohini_channels::matrix::MatrixAdapter;
use mohini_channels::mattermost::MattermostAdapter;
use mohini_channels::rocketchat::RocketChatAdapter;
use mohini_channels::router::AgentRouter;
use mohini_channels::signal::SignalAdapter;
use mohini_channels::slack::SlackAdapter;
use mohini_channels::teams::TeamsAdapter;
use mohini_channels::telegram::TelegramAdapter;
use mohini_channels::twitch::TwitchAdapter;
use mohini_channels::types::ChannelAdapter;
use mohini_channels::whatsapp::WhatsAppAdapter;
use mohini_channels::xmpp::XmppAdapter;
use mohini_channels::zulip::ZulipAdapter;
// Wave 3
use mohini_channels::bluesky::BlueskyAdapter;
use mohini_channels::feishu::FeishuAdapter;
use mohini_channels::line::LineAdapter;
use mohini_channels::mastodon::MastodonAdapter;
use mohini_channels::messenger::MessengerAdapter;
use mohini_channels::reddit::RedditAdapter;
use mohini_channels::revolt::RevoltAdapter;
use mohini_channels::viber::ViberAdapter;
// Wave 4
use mohini_channels::flock::FlockAdapter;
use mohini_channels::guilded::GuildedAdapter;
use mohini_channels::keybase::KeybaseAdapter;
use mohini_channels::nextcloud::NextcloudAdapter;
use mohini_channels::nostr::NostrAdapter;
use mohini_channels::pumble::PumbleAdapter;
use mohini_channels::threema::ThreemaAdapter;
use mohini_channels::twist::TwistAdapter;
use mohini_channels::webex::WebexAdapter;
// Wave 5
use async_trait::async_trait;
use mohini_channels::dingtalk::DingTalkAdapter;
use mohini_channels::discourse::DiscourseAdapter;
use mohini_channels::gitter::GitterAdapter;
use mohini_channels::gotify::GotifyAdapter;
use mohini_channels::linkedin::LinkedInAdapter;
use mohini_channels::mumble::MumbleAdapter;
use mohini_channels::ntfy::NtfyAdapter;
use mohini_channels::webhook::WebhookAdapter;
...
use mohini_runtime::str_utils::safe_truncate_str;

/// Wraps `MohiniKernel` to implement `ChannelBridgeHandle`.
pub struct KernelBridgeAdapter {
    kernel: Arc<MohiniKernel>,
    started_at: Instant,
}
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn send_message_with_blocks(
        ...
        let text: String = blocks
            ...
            .filter_map(|b| match b {
                mohini_types::message::ContentBlock::Text { text, .. } => Some(text.as_str()),
                _ => None,
            })
            .collect::<Vec<_>>()
            .join("\n");
    ...
    async fn spawn_agent_by_name(&self, manifest_name: &str) -> Result<AgentId, String> {
        // Look for manifest at ~/.mohini/agents/{name}/agent.toml
        ...
        let manifest: mohini_types::agent::AgentManifest =
    ...
    async fn uptime_info(&self) -> String {
        ...
        if hours > 0 {
            format!(
                "Mohini status: {}h {}m uptime, {} agent(s)",
                hours,
                mins,
                agents.len()
            )
            ...
            format!(
                "Mohini status: {}m uptime, {} agent(s)",
                mins,
                agents.len()
            )
        }
    ...
    async fn list_providers_text(&self) -> String {
        ...
        for p in catalog.list_providers() {
            let status = match p.auth_status {
                mohini_types::model_catalog::AuthStatus::Configured => "configured",
                mohini_types::model_catalog::AuthStatus::Missing => "not configured",
                mohini_types::model_catalog::AuthStatus::NotRequired => "local (no key needed)",
            };
            msg.push_str(&format!(
                "  {} — {} [{}, {} model(s)]\n",
    ...
    async fn list_skills_text(&self) -> String {
        ...
        if skills.is_empty() {
            return "No skills installed. Place skills in ~/.mohini/skills/ or install from the marketplace.".to_string();
        }
        ...
        let result = self
            ...
            .execute_run(
                ...
                |step_agent| match step_agent {
                    mohini_kernel::workflow::StepAgent::ById { id } => {
                        let aid: AgentId = id.parse().ok()?;
                        let entry = registry_ref.get(aid)?;
                        Some((aid, entry.name.clone()))
                    }
                    mohini_kernel::workflow::StepAgent::ByName { name } => {
    ...
    async fn list_schedules_text(&self) -> String {
        ...
        for job in &jobs {
            ...
            let sched = match &job.schedule {
                mohini_types::scheduler::CronSchedule::Cron { expr, .. } => expr.clone(),
                mohini_types::scheduler::CronSchedule::Every { every_secs } => {
                    format!("every {every_secs}s")
                }
                mohini_types::scheduler::CronSchedule::At { at } => {
    ...
    async fn manage_schedule_text(&self, action: &str, args: &[String]) -> String {
        match action {
            "add" => {
                ...
                let job = mohini_types::scheduler::CronJob {
                    id: mohini_types::scheduler::CronJobId::new(),
                    agent_id: agent.id,
                    name: format!("chat-{}", &agent.name),
                    enabled: true,
                    schedule: mohini_types::scheduler::CronSchedule::Cron {
                        expr: cron_expr.clone(),
                        tz: None,
                    },
                    action: mohini_types::scheduler::CronAction::AgentTurn {
                        message: message.clone(),
                        model_override: None,
                        timeout_secs: None,
                    },
                    delivery: mohini_types::scheduler::CronDelivery::None,
    ...
    async fn channel_overrides(
        &self,
        channel_type: &str,
    ) -> Option<mohini_types::config::ChannelOverrides> {
    ...
    async fn authorize_channel_user(
        ...
        let auth_action = match action {
            "chat" => mohini_kernel::auth::Action::ChatWithAgent,
            "spawn" => mohini_kernel::auth::Action::SpawnAgent,
            "kill" => mohini_kernel::auth::Action::KillAgent,
            "install_skill" => mohini_kernel::auth::Action::InstallSkill,
            _ => mohini_kernel::auth::Action::ChatWithAgent,
        };
    ...
    async fn record_delivery(
        ...
        let receipt = if success {
            mohini_kernel::DeliveryTracker::sent_receipt(channel, recipient)
        } else {
            mohini_kernel::DeliveryTracker::failed_receipt(
                channel,
                recipient,
    ...
    async fn peers_text(&self) -> String {
        if !self.kernel.config.network_enabled {
            return "MMP peer network is disabled. Set network_enabled = true in config.toml."
                .to_string();
        }
        match &self.kernel.peer_registry {
            Some(registry) => {
                ...
                if peers.is_empty() {
                    "MMP network enabled but no peers connected.".to_string()
                } else {
                    let mut msg = format!("MMP Peers ({} connected):\n", peers.len());
                    for p in &peers {
                        msg.push_str(&format!(
                            "  {} — {} ({:?})\n",
            ...
            None => "MMP peer node not started.".to_string(),
        }
    }
    ...
    async fn a2a_agents_text(&self) -> String {
        ...
        for (url, card) in agents.iter() {
            ...
            if !desc.is_empty() {
                let short = mohini_types::truncate_str(desc, 60);
...
fn parse_trigger_pattern(s: &str) -> Option<mohini_kernel::triggers::TriggerPattern> {
    use mohini_kernel::triggers::TriggerPattern;
...
pub async fn start_channel_bridge(kernel: Arc<MohiniKernel>) -> Option<BridgeManager> {
...
pub async fn start_channel_bridge_with_config(
    ...
    if let Some(ref sig_config) = config.signal {
        if !sig_config.phone_number.is_empty() {
            let adapter = Arc::new(SignalAdapter::from_url_and_phone(
...
pub async fn reload_channels_from_disk(
    ...
    let fresh_config = mohini_kernel::config::load_config(Some(&config_path));
