...
//!
//! Allows any OpenAI-compatible client library to talk to Mohini agents.
//! The `model` field resolves to an agent (by name, UUID, or `mohini:<name>`),
...
use mohini_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "mohini:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("mohini:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("mohini:{}", e.name),
            object: "model",
            created,
            owned_by: "mohini".to_string(),
        })
        .collect();
