//! Route handlers for the Mohini API.
...
use axum::Json;
use dashmap::DashMap;
use mohini_kernel::triggers::{TriggerId, TriggerPattern};
use mohini_kernel::workflow::{
    ErrorMode, StepAgent, StepMode, Workflow, WorkflowId, WorkflowStep,
};
use mohini_kernel::MohiniKernel;
use mohini_runtime::kernel_handle::KernelHandle;
use mohini_runtime::tool_runner::builtin_tool_definitions;
use mohini_types::agent::{AgentId, AgentIdentity, AgentManifest};
...
pub struct AppState {
    pub kernel: Arc<MohiniKernel>,
    pub started_at: Instant,
    /// Optional peer registry for MMP mesh networking status.
    pub peer_registry: Option<Arc<mohini_wire::registry::PeerRegistry>>,
    /// Channel bridge manager — held behind a Mutex so it can be swapped on hot-reload.
    pub bridge_manager: tokio::sync::Mutex<Option<mohini_channels::bridge::BridgeManager>>,
    /// Live channel config — updated on every hot-reload so list_channels() reflects reality.
    pub channels_config: tokio::sync::RwLock<mohini_types::config::ChannelsConfig>,
    /// Notify handle to trigger graceful HTTP server shutdown from the API.
    pub shutdown_notify: Arc<tokio::sync::Notify>,
    /// SkillHub response cache — prevents 429 rate limiting on rapid dashboard refreshes.
    ...
    pub provider_probe_cache: mohini_runtime::provider_health::ProbeCache,
}
        ...
        match state.kernel.verify_signed_manifest(signed_json) {
            ...
            Err(e) => {
                ...
                state.kernel.audit_log.record(
                    "system",
                    mohini_runtime::audit::AuditAction::AuthAttempt,
...
pub async fn list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let agents: Vec<serde_json::Value> = state
        ...
        .map(|e| {
            ...
            let ready = matches!(e.state, mohini_types::agent::AgentState::Running)
                && auth_status != "missing";
...
) -> Vec<mohini_types::message::ContentBlock> {
    use base64::Engine;

    let upload_dir = std::env::temp_dir().join("mohini_uploads");
    ...
    for att in attachments {
        ...
        match std::fs::read(&file_path) {
            Ok(data) => {
                ...
                blocks.push(mohini_types::message::ContentBlock::Image {
...
pub fn inject_attachments_into_session(
    kernel: &MohiniKernel,
    agent_id: AgentId,
    image_blocks: Vec<mohini_types::message::ContentBlock>,
) {
    use mohini_types::message::{Message, MessageContent, Role};
    ...
    let mut session = match kernel.memory.get_session(entry.session_id) {
        Ok(Some(s)) => s,
        _ => mohini_memory::session::Session {
            id: entry.session_id,
            agent_id,
            messages: Vec::new(),
        ...
        Ok(Some(session)) => {
            ...
            for m in &session.messages {
                ...
                let content = match &m.content {
                    mohini_types::message::MessageContent::Text(t) => t.clone(),
                    mohini_types::message::MessageContent::Blocks(blocks) => {
                        let mut texts = Vec::new();
                        for b in blocks {
                            match b {
                                mohini_types::message::ContentBlock::Text { text, .. } => {
                                    texts.push(text.clone());
                                }
                                mohini_types::message::ContentBlock::Image {
                                    media_type,
                                    data,
                                } => {
                                    ...
                                    let upload_dir =
                                        std::env::temp_dir().join("mohini_uploads");
                                ...
                                mohini_types::message::ContentBlock::ToolUse {
                                    id,
                                    name,
                                    input,
                                    ..
                                ...
                                mohini_types::message::ContentBlock::ToolResult { .. } => {}
                                _ => {}
                            }
                        }
                        texts.join("\n")
            ...
            for m in &session.messages {
                if let mohini_types::message::MessageContent::Blocks(blocks) = &m.content {
                    for b in blocks {
                        if let mohini_types::message::ContentBlock::ToolResult {
                            tool_use_id,
                            content: result,
                            is_error,
                            ..
...
pub async fn shutdown(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    state.kernel.audit_log.record(
        "system",
        mohini_runtime::audit::AuditAction::ConfigChange,
...
pub async fn list_profiles() -> impl IntoResponse {
    use mohini_types::agent::ToolProfile;
...
pub async fn send_message_stream(
    ...
    use futures::stream;
    use mohini_runtime::llm_driver::StreamEvent;
...
const CHANNEL_REGISTRY: &[ChannelMeta] = &[
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "user_id", label: "Bot User ID", field_type: FieldType::Text, env_var: None, required: false, placeholder: "@mohini:matrix.org", advanced: true },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "mohini_bot", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Bot Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "mohini_bot", advanced: false },
            ...
            ChannelField { key: "subreddits", label: "Subreddits", field_type: FieldType::List, env_var: None, required: false, placeholder: "mohini, rust", advanced: true },
            ChannelField { key: "default_agent", label: "Default Agent", field_type: FieldType::Text, env_var: None, required: false, placeholder: "assistant", advanced: true },
        ],
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "mohini", advanced: false },
            ChannelField { key: "channels", label: "Channels", field_type: FieldType::List, env_var: None, required: false, placeholder: "#mohini, #general", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Bot Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "mohini", advanced: false },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "topic", label: "Topic", field_type: FieldType::Text, env_var: None, required: true, placeholder: "mohini-alerts", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "mohini", advanced: false },
...
fn is_channel_configured(config: &mohini_types::config::ChannelsConfig, name: &str) -> bool {
    match name {
...
fn channel_config_values(
    config: &mohini_types::config::ChannelsConfig,
    name: &str,
...
pub async fn configure_channel(
    ...
    let home = mohini_kernel::config::mohini_home();
...
async fn send_channel_test_message(channel_name: &str, target_id: &str) -> Result<(), String> {
    ...
    let test_msg = "Mohini test message — your channel is connected!";
...
// ---------------------------------------------------------------------------
// WhatsApp QR login flow (LegacyImport-style)
// ---------------------------------------------------------------------------
...
pub async fn list_templates() -> impl IntoResponse {
    let agents_dir = mohini_kernel::config::mohini_home().join("agents");
...
pub async fn get_agent_kv(
    ...
    let agent_id = mohini_kernel::kernel::shared_memory_agent_id();
...
pub async fn health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let shared_id = mohini_types::agent::AgentId(uuid::Uuid::from_bytes([
...
///
/// Returns counters and gauges for monitoring Mohini in production:
/// - `mohini_agents_active` — number of active agents
/// - `mohini_uptime_seconds` — seconds since daemon started
/// - `mohini_tokens_total` — total tokens consumed (per agent)
/// - `mohini_tool_calls_total` — total tool calls (per agent)
/// - `mohini_panics_total` — supervisor panic count
/// - `mohini_restarts_total` — supervisor restart count
pub async fn prometheus_metrics(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let mut out = String::with_capacity(2048);

    // Uptime
    let uptime = state.started_at.elapsed().as_secs();
    out.push_str("# HELP mohini_uptime_seconds Time since daemon started.\n");
    out.push_str("# TYPE mohini_uptime_seconds gauge\n");
    out.push_str(&format!("mohini_uptime_seconds {uptime}\n\n"));

    // Active agents
    let agents = state.kernel.registry.list();
    let active = agents
        .iter()
        .filter(|a| matches!(a.state, mohini_types::agent::AgentState::Running))
        .count();
    out.push_str("# HELP mohini_agents_active Number of active agents.\n");
    out.push_str("# TYPE mohini_agents_active gauge\n");
    out.push_str(&format!("mohini_agents_active {active}\n"));
    out.push_str("# HELP mohini_agents_total Total number of registered agents.\n");
    out.push_str("# TYPE mohini_agents_total gauge\n");
    out.push_str(&format!("mohini_agents_total {}\n\n", agents.len()));

    // Per-agent token and tool usage
    out.push_str("# HELP mohini_tokens_total Total tokens consumed (rolling hourly window).\n");
    out.push_str("# TYPE mohini_tokens_total gauge\n");
    out.push_str("# HELP mohini_tool_calls_total Total tool calls (rolling hourly window).\n");
    out.push_str("# TYPE mohini_tool_calls_total gauge\n");
    for agent in &agents {
        let name = &agent.name;
        ...
        if let Some((tokens, tools)) = state.kernel.scheduler.get_usage(agent.id) {
            out.push_str(&format!(
                "mohini_tokens_total{{agent=\"{name}\",provider=\"{provider}\",model=\"{model}\"}} {tokens}\n"
            ));
            out.push_str(&format!(
                "mohini_tool_calls_total{{agent=\"{name}\"}} {tools}\n"
            ));
        }
    }
    out.push('\n');
    ...
    // Supervisor health
    let health = state.kernel.supervisor.health();
    out.push_str("# HELP mohini_panics_total Total supervisor panics since start.\n");
    out.push_str("# TYPE mohini_panics_total counter\n");
    out.push_str(&format!("mohini_panics_total {}\n", health.panic_count));
    out.push_str("# HELP mohini_restarts_total Total supervisor restarts since start.\n");
    out.push_str("# TYPE mohini_restarts_total counter\n");
    out.push_str(&format!(
        "mohini_restarts_total {}\n\n",
        health.restart_count
    ));

    // Version info
    out.push_str("# HELP mohini_info Mohini version and build info.\n");
    out.push_str("# TYPE mohini_info gauge\n");
    out.push_str(&format!(
        "mohini_info{{version=\"{}\"}} 1\n",
    ...
    let skills: Vec<serde_json::Value> = registry
        ...
        .map(|s| {
            let source = match &s.manifest.source {
                Some(mohini_skills::SkillSource::SkillHub { slug, version }) => {
                    serde_json::json!({"type": "skillhub", "slug": slug, "version": version})
                }
                Some(mohini_skills::SkillSource::LegacyImport) => {
                    serde_json::json!({"type": "legacy_import"})
                }
                Some(mohini_skills::SkillSource::Bundled) => {
                    serde_json::json!({"type": "bundled"})
                }
                Some(mohini_skills::SkillSource::Native) | None => {
...
/// POST /api/skills/install — Install a skill from SkillHub (GitHub).
pub async fn install_skill(
    State(state): State<Arc<AppState>>,
    Json(req): Json<SkillInstallRequest>,
) -> impl IntoResponse {
    let skills_dir = state.kernel.config.home_dir.join("skills");
    let config = mohini_skills::marketplace::MarketplaceConfig::default();
    let client = mohini_skills::marketplace::MarketplaceClient::new(config);
...
/// GET /api/marketplace/search — Search the SkillHub marketplace.
...
// ---------------------------------------------------------------------------
// SkillHub (LegacyImport ecosystem) endpoints
// ---------------------------------------------------------------------------

/// GET /api/skillhub/search — Search SkillHub skills using vector/semantic search.
///
/// Query parameters:
...
pub async fn skillhub_search(
    ...
    let client = mohini_skills::hub::SkillHubClient::new(cache_dir);
    ...
    match client.search(&query, limit).await {
        Ok(results) => {
            ...
            state.skillhub_cache.insert(cache_key, (Instant::now(), resp.clone()));
            (StatusCode::OK, Json(resp))
        }
        Err(e) => {
            let msg = format!("{e}");
            tracing::warn!("SkillHub search failed: {msg}");
            let status = if is_skillhub_rate_limit(&e) {
                StatusCode::TOO_MANY_REQUESTS
            } else {
                StatusCode::OK
            };
...
/// GET /api/skillhub/browse — Browse SkillHub skills by sort order.
///
/// Query parameters:
...
pub async fn skillhub_browse(
    ...
    let sort = match params.get("sort").map(|s| s.as_str()) {
        Some("downloads") => mohini_skills::hub::SkillHubSort::Downloads,
        Some("stars") => mohini_skills::hub::SkillHubSort::Stars,
        Some("updated") => mohini_skills::hub::SkillHubSort::Updated,
        Some("rating") => mohini_skills::hub::SkillHubSort::Rating,
        _ => mohini_skills::hub::SkillHubSort::Trending,
    };
    ...
    match client.browse(sort, limit, cursor).await {
        Ok(results) => {
            let items: Vec<serde_json::Value> = results
                .items
                .iter()
                .map(skillhub_browse_entry_to_json)
                .collect();
            let resp = serde_json::json!({
                "items": items,
...
/// GET /api/skillhub/skill/{slug}/code — Fetch the source code (SKILL.md) of a SkillHub skill.
...
pub async fn skillhub_install(
    State(state): State<Arc<AppState>>,
    Json(req): Json<crate::types::SkillHubInstallRequest>,
) -> impl IntoResponse {
    let skills_dir = state.kernel.config.home_dir.join("skills");
    let cache_dir = state.kernel.config.home_dir.join(".cache").join("skillhub");
    let client = mohini_skills::hub::SkillHubClient::new(cache_dir);
    ...
    match client.install(&req.slug, &skills_dir).await {
        ...
        Err(e) => {
            let msg = format!("{e}");
            let status = if matches!(e, mohini_skills::SkillError::SecurityBlocked(_)) {
                StatusCode::FORBIDDEN
            } else if is_skillhub_rate_limit(&e) {
                StatusCode::TOO_MANY_REQUESTS
            } else if matches!(e, mohini_skills::SkillError::Network(_)) {
...
/// Check whether a SkillError represents a SkillHub rate-limit (429).
fn is_skillhub_rate_limit(err: &mohini_skills::SkillError) -> bool {
    matches!(err, mohini_skills::SkillError::RateLimited(_))
}

/// Convert a browse entry (nested stats/tags) to a flat JSON object for the frontend.
fn skillhub_browse_entry_to_json(
    entry: &mohini_skills::hub::SkillHubBrowseEntry,
) -> serde_json::Value {
    let version = mohini_skills::hub::SkillHubClient::entry_version(entry);
    serde_json::json!({
        "slug": entry.slug,
...
pub async fn activate_hand(
    State(state): State<Arc<AppState>>,
    Path(hand_id): Path<String>,
    body: Option<Json<mohini_hands::ActivateHandRequest>>,
) -> impl IntoResponse {
            ...
            if let Some(agent_id) = instance.agent_id {
                ...
                if let Some(entry) = entry {
                    if !matches!(
                        ...
                        mohini_types::agent::ScheduleMode::Reactive
                    ) {
...
pub async fn hand_stats(
    ...
    let shared_id = mohini_kernel::kernel::shared_memory_agent_id();
...
pub async fn hand_instance_browser(
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            mohini_runtime::browser::BrowserCommand::ReadPage,
        )
        .await
    {
        Ok(resp) if resp.success => {
            if let Some(data) = &resp.data {
                ...
                if content.len() > 2000 {
                    content = format!("{}... (truncated)", mohini_types::truncate_str(&content, 2000));
                }
            }
        }
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            mohini_runtime::browser::BrowserCommand::Screenshot,
        )
        .await
    {
        Ok(resp) if resp.success => {
...
/// GET /api/peers — List known MMP peers.
...
/// GET /api/network/status — MMP network status summary.
...
pub async fn update_budget(
    ...
    let config_ptr = &state.kernel.config as *const mohini_types::config::KernelConfig
        as *mut mohini_types::config::KernelConfig;
...
pub async fn agent_budget_status(
    ...
    let usage_store = mohini_memory::usage::UsageStore::new(state.kernel.memory.usage_conn());
...
pub async fn delete_session(
    ...
    let session_id = match id.parse::<uuid::Uuid>() {
        Ok(u) => mohini_types::agent::SessionId(u),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn set_session_label(
    ...
    if let Some(lbl) = label {
        if let Err(e) = mohini_types::agent::SessionLabel::new(lbl) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": e.to_string()})),
            );
...
pub async fn find_session_by_label(
    ...
    let agent_id = match agent_id_str.parse::<uuid::Uuid>() {
        Ok(u) => mohini_types::agent::AgentId(u),
        Err(_) => {
            // Try name lookup
...
/// GET /api/migrate/detect — Auto-detect LegacyImport installation.
pub async fn migrate_detect() -> impl IntoResponse {
    match mohini_migrate::legacy_import::detect_openclaw_home() {
        Some(path) => {
            let scan = mohini_migrate::legacy_import::scan_openclaw_workspace(&path);
            (
                StatusCode::OK,
                Json(serde_json::json!({
                    "detected": true,
...
/// POST /api/migrate/scan — Scan a specific directory for LegacyImport workspace.
...
pub async fn run_migrate(Json(req): Json<MigrateRequest>) -> impl IntoResponse {
    let source = match req.source.as_str() {
        "legacy_import" => mohini_migrate::MigrateSource::LegacyImport,
        "langchain" => mohini_migrate::MigrateSource::LangChain,
        "autogpt" => mohini_migrate::MigrateSource::AutoGpt,
        other => {
            return (
                StatusCode::BAD_REQUEST,
                Json(
                    serde_json::json!({"error": format!("Unknown source: {other}. Use 'legacy_import', 'langchain', or 'autogpt'")}),
                ),
            );
        }
    };
    ...
    let models: Vec<serde_json::Value> = catalog
        ...
        .filter(|m| {
            ...
            if available_only {
                ...
                if let Some(p) = provider {
                    if p.auth_status == mohini_types::model_catalog::AuthStatus::Missing {
                        return false;
                    }
                }
            }
        ...
        .map(|m| {
            ...
            let available = catalog
                ...
                .map(|p| p.auth_status != mohini_types::model_catalog::AuthStatus::Missing)
                .unwrap_or(m.tier == mohini_types::model_catalog::ModelTier::Custom);
            serde_json::json!({
                "id": m.id,
...
pub async fn list_providers(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let provider_list: Vec<mohini_types::model_catalog::ProviderInfo> = {
        let catalog = state
            .kernel
            .model_catalog
            .read()
    ...
    let probe_futures: Vec<_> = local_providers
        .iter()
        .map(|(_, id, url)| {
            mohini_runtime::provider_health::probe_provider_cached(id, url, cache)
        })
        .collect();
    ...
    let mut probe_map: HashMap<usize, mohini_runtime::provider_health::ProbeResult> =
...
///
/// Persists to `~/.mohini/custom_models.json` and makes the model immediately
...
pub async fn add_custom_model(
    ...
    let entry = mohini_types::model_catalog::ModelCatalogEntry {
        id: id.clone(),
        display_name: display,
        provider: provider.clone(),
        tier: mohini_types::model_catalog::ModelTier::Custom,
        context_window,
...
pub async fn a2a_agent_card(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if let Some(first) = agents.first() {
        let card = mohini_runtime::a2a::build_agent_card(&first.manifest, &base_url);
        (
            StatusCode::OK,
        ...
        let card = serde_json::json!({
            "name": "mohini",
            "description": "Mohini Agent OS — no agents spawned yet",
...
pub async fn a2a_list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let cards: Vec<serde_json::Value> = agents
        .iter()
        .map(|entry| {
            let card = mohini_runtime::a2a::build_agent_card(&entry.manifest, &base_url);
...
pub async fn a2a_send_task(
    ...
    let task = mohini_runtime::a2a::A2aTask {
        id: task_id.clone(),
        session_id: session_id.clone(),
        status: mohini_runtime::a2a::A2aTaskStatus::Working.into(),
        messages: vec![mohini_runtime::a2a::A2aMessage {
            role: "user".to_string(),
            parts: vec![mohini_runtime::a2a::A2aPart::Text {
    ...
    match state.kernel.send_message(agent.id, &message_text).await {
        Ok(result) => {
            let response_msg = mohini_runtime::a2a::A2aMessage {
        ...
        Err(e) => {
            let error_msg = mohini_runtime::a2a::A2aMessage {
                role: "agent".to_string(),
                parts: vec![mohini_runtime::a2a::A2aPart::Text {
                    text: format!("Error: {e}"),
                }],
...
pub async fn a2a_discover_external(
    ...
    let client = mohini_runtime::a2a::A2aClient::new();
...
pub async fn mcp_http(
    ...
    let mut tools = builtin_tool_definitions();
    {
        ...
        for skill_tool in registry.all_tool_definitions() {
            tools.push(mohini_types::tool::ToolDefinition {
    ...
    if method == "tools/call" {
        ...
        let kernel_handle: Arc<dyn mohini_runtime::kernel_handle::KernelHandle> =
            state.kernel.clone() as Arc<dyn mohini_runtime::kernel_handle::KernelHandle>;
        let result = mohini_runtime::tool_runner::execute_tool(
            "mcp-http",
            tool_name,
            &arguments,
            Some(&kernel_handle),
    ...
    let response = mohini_runtime::mcp_server::handle_mcp_request(&request, &tools).await;
    Json(response)
}
...
pub async fn switch_agent_session(
    ...
    let session_id = match session_id_str.parse::<uuid::Uuid>() {
        Ok(uuid) => mohini_types::agent::SessionId(uuid),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid session ID"})),
...
pub async fn get_agent_mcp_servers(
    ...
    if let Ok(mcp_tools) = state.kernel.mcp_tools.lock() {
        ...
        for tool in mcp_tools.iter() {
            if let Some(server) = mohini_runtime::mcp::extract_mcp_server(&tool.name) {
...
///
/// SECURITY: Writes to `~/.mohini/secrets.env`, sets env var in process,
...
pub async fn set_provider_key(
    ...
    let switched = if !current_has_key && current_provider != name {
        ...
        if let Some(model_id) = default_model {
            ...
            {
                let new_dm = mohini_types::config::DefaultModelConfig {
                    provider: name.clone(),
                    model: model_id,
        ...
        if needs_update {
            ...
            *guard = Some(mohini_types::config::DefaultModelConfig {
                api_key_env: env_var.clone(),
                ..base
            });
        }
...
pub async fn test_provider(
    ...
    let driver_config = mohini_runtime::llm_driver::DriverConfig {
        provider: name.clone(),
        api_key,
    ...
    match mohini_runtime::drivers::create_driver(&driver_config) {
        Ok(driver) => {
            // Send a minimal completion request to test connectivity
            let test_req = mohini_runtime::llm_driver::CompletionRequest {
                model: default_model.clone(),
                messages: vec![mohini_types::message::Message::user("Hi")],
                tools: vec![],
                max_tokens: 1,
                temperature: 0.0,
                system: None,
...
pub async fn set_provider_url(
    ...
    let probe =
        mohini_runtime::provider_health::probe_provider(&name, &base_url).await;
...
pub async fn create_skill(
    ...
    // Write skill.toml to ~/.mohini/skills/{name}/
    ...
    for info in registry.list_all_info() {
        ...
        let status = match &info.installed {
            ...
            Some(_) => match h.as_ref().map(|h| &h.status) {
                Some(mohini_extensions::IntegrationStatus::Ready) => "ready",
                Some(mohini_extensions::IntegrationStatus::Error(_)) => "error",
                _ => "installed",
            },
...
pub async fn add_integration(
    ...
    let install_err = {
        ...
        if registry.is_installed(&id) {
            ...
            let entry = mohini_extensions::InstalledIntegration {
                id: id.clone(),
...
/// Must match the value in `mohini-kernel/src/kernel.rs::shared_memory_agent_id()`.
...
const SCHEDULES_KEY: &str = "__mohini_schedules";
...
pub struct PatchAgentConfigRequest {
    ...
    pub fallback_models: Option<Vec<mohini_types::agent::FallbackModel>>,
}
...
pub async fn upload_file(
    ...
    let transcription = if content_type.starts_with("audio/") {
        let attachment = mohini_types::media::MediaAttachment {
            media_type: mohini_types::media::MediaType::Audio,
            mime_type: content_type.clone(),
            source: mohini_types::media::MediaSource::FilePath {
...
pub async fn serve_upload(Path(file_id): Path<String>) -> impl IntoResponse {
    ...
    let file_path = std::env::temp_dir().join("mohini_uploads").join(&file_id);
...
pub async fn create_approval(
    ...
    use mohini_types::approval::{ApprovalRequest, RiskLevel};
...
pub async fn approve_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        mohini_types::approval::ApprovalDecision::Approved,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn reject_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        mohini_types::approval::ApprovalDecision::Denied,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn delete_cron_job(
    ...
    match uuid::Uuid::parse_str(&id) {
        Ok(uuid) => {
            let job_id = mohini_types::scheduler::CronJobId(uuid);
...
pub async fn webhook_wake(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<mohini_types::webhook::WakePayload>,
) -> impl IntoResponse {
...
pub async fn webhook_agent(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<mohini_types::webhook::AgentHookPayload>,
) -> impl IntoResponse {
    // Check if webhook triggers are enabled
    let wh_config = match &state.kernel.config.webhook_triggers {
        Some(c) if c.enabled => c,
...
pub async fn add_binding(
    State(state): State<Arc<AppState>>,
    Json(binding): Json<mohini_types::config::AgentBinding>,
) -> impl IntoResponse {
...
pub async fn pairing_request(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match state.kernel.pairing.create_pairing_request() {
        Ok(req) => {
            let qr_uri = format!("mohini://pair?token={}", req.token);
            Json(serde_json::json!({
                "token": req.token,
                "qr_uri": qr_uri,
...
pub async fn pairing_complete(
    ...
    let push_token = body
        ...
        .and_then(|v| v.as_str())
        .map(String::from);
    let device_info = mohini_kernel::pairing::PairedDevice {
...
pub async fn copilot_oauth_start() -> impl IntoResponse {
    ...
    match mohini_runtime::copilot_oauth::start_device_flow().await {
        Ok(resp) => {
...
pub async fn copilot_oauth_poll(
    ...
    match mohini_runtime::copilot_oauth::poll_device_flow(&device_code).await {
        mohini_runtime::copilot_oauth::DeviceFlowStatus::Pending => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "pending"})),
        ),
        mohini_runtime::copilot_oauth::DeviceFlowStatus::Complete { access_token } => {
            // Save to secrets.env
            let secrets_path = state.kernel.config.home_dir.join("secrets.env");
        ...
        mohini_runtime::copilot_oauth::DeviceFlowStatus::SlowDown { new_interval } => {
            // Update interval
        ...
        mohini_runtime::copilot_oauth::DeviceFlowStatus::Expired => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
                Json(serde_json::json!({"status": "expired"})),
        ...
        mohini_runtime::copilot_oauth::DeviceFlowStatus::AccessDenied => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
        ...
        mohini_runtime::copilot_oauth::DeviceFlowStatus::Error(e) => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "error", "error": e})),
        ),
    }
...
pub async fn comms_topology(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    use mohini_types::comms::{EdgeKind, TopoEdge, TopoNode, Topology};
    ...
    for event in &events {
        if let mohini_types::event::EventPayload::Message(_) = &event.payload {
            if let mohini_types::event::EventTarget::Agent(target_id) = &event.target {
...
fn filter_to_comms_event(
    event: &mohini_types::event::Event,
    agents: &[mohini_types::agent::AgentEntry],
) -> Option<mohini_types::comms::CommsEvent> {
    use mohini_types::comms::{CommsEvent, CommsEventKind};
    use mohini_types::event::{EventPayload, EventTarget, LifecycleEvent};
    ...
    match &event.payload {
        EventPayload::Message(msg) => {
            ...
            Some(CommsEvent {
                ...
                detail: mohini_types::truncate_str(&msg.content, 200).to_string(),
            })
        }
...
fn audit_to_comms_event(
    entry: &mohini_runtime::audit::AuditEntry,
    agents: &[mohini_types::agent::AgentEntry],
) -> Option<mohini_types::comms::CommsEvent> {
    use mohini_types::comms::{CommsEvent, CommsEventKind};
        ...
        agents
            ...
            .unwrap_or_else(|| {
                if id.is_empty() || id == "system" {
                    ...
                    mohini_types::truncate_str(id, 12).to_string()
                }
            })
    };
        ...
        "AgentMessage" => {
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!("{} in / {} out — {}", in_tok, out_tok, mohini_types::truncate_str(&entry.outcome, 80))
                }
            } else if entry.outcome != "ok" {
                format!("{} — {}", mohini_types::truncate_str(&entry.detail, 80), mohini_types::truncate_str(&entry.outcome, 80))
            } else {
                mohini_types::truncate_str(&entry.detail, 200).to_string()
            };
            (CommsEventKind::AgentMessage, detail, "user")
        }
        "AgentSpawn" => (
            CommsEventKind::AgentSpawned,
            format!("Agent spawned: {}", mohini_types::truncate_str(&entry.detail, 100)),
            "",
        ),
        "AgentKill" => (
            CommsEventKind::AgentTerminated,
            format!("Agent killed: {}", mohini_types::truncate_str(&entry.detail, 100)),
            "",
        ),
        _ => return None,
    };
...
pub async fn comms_events(
    ...
    let mut comms_events: Vec<mohini_types::comms::CommsEvent> = bus_events
        .iter()
...
pub async fn comms_send(
    State(state): State<Arc<AppState>>,
    Json(req): Json<mohini_types::comms::CommsSendRequest>,
) -> impl IntoResponse {
    // Validate from agent exists
    let from_id: mohini_types::agent::AgentId = match req.from_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    let to_id: mohini_types::agent::AgentId = match req.to_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn comms_task(
    State(state): State<Arc<AppState>>,
    Json(req): Json<mohini_types::comms::CommsTaskRequest>,
) -> impl IntoResponse {
