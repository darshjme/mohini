//! Mohini daemon server — boots the kernel and serves the HTTP API.
...
/// Daemon info written to `~/.mohini/daemon.json` so the CLI can find us.
...
///
/// This is extracted from `run_daemon()` so that embedders (e.g. mohini-desktop)
...
pub async fn build_router(
    ...
    let state = Arc::new(AppState {
        ...
        provider_probe_cache: mohini_runtime::provider_health::ProbeCache::new(),
    });
    ...
    let app = Router::new()
        ...
        .route(
            "/api/skillhub/search",
            axum::routing::get(routes::skillhub_search),
        )
        .route(
            "/api/skillhub/browse",
            axum::routing::get(routes::skillhub_browse),
        )
        .route(
            "/api/skillhub/skill/{slug}",
            axum::routing::get(routes::skillhub_skill_detail),
        )
        .route(
            "/api/skillhub/skill/{slug}/code",
            axum::routing::get(routes::skillhub_skill_code),
        )
        .route(
            "/api/skillhub/install",
            axum::routing::post(routes::skillhub_install),
        )
        // Hands endpoints
...
/// Start the Mohini daemon: boot kernel + HTTP API server.
///
...
pub async fn run_daemon(
    ...
    info!("Mohini API server listening on http://{addr}");
...
/// Check if an Mohini daemon is actually responding at the given address.
