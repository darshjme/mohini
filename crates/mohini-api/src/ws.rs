...
async fn handle_command(
    ...
    match cmd {
        ...
        "context" => match state.kernel.context_report(agent_id) {
            Ok(report) => {
                let formatted = mohini_runtime::compactor::format_context_report(&report);
                serde_json::json!({
                    "type": "command_result",
                    "command": cmd,
                    "message": formatted,
        ...
        "peers" => {
            let msg = if !state.kernel.config.network_enabled {
                "MMP network disabled.".to_string()
            } else {
...
///
/// Uses the proper LLM error classifier from `mohini_runtime::llm_errors`
/// for comprehensive 20-provider coverage with actionable advice.
fn classify_streaming_error(err: &mohini_kernel::error::KernelError) -> String {
