// Mohini App — Alpine.js init, hash router, global store
'use strict';
...
document.addEventListener('alpine:init', function() {
  ...
  var savedKey = localStorage.getItem('mohini-api-key');
  if (savedKey) MohiniAPI.setAuthToken(savedKey);
  ...
  Alpine.store('app', {
    ...
    agentCount: 0,
    pendingAgent: null,
    focusMode: localStorage.getItem('mohini-focus') === 'true',
    showOnboarding: false,
    showAuthPrompt: false,

    toggleFocusMode() {
      this.focusMode = !this.focusMode;
      localStorage.setItem('mohini-focus', this.focusMode);
    },

    async refreshAgents() {
      try {
        var agents = await MohiniAPI.get('/api/agents');
    ...
    async checkStatus() {
      try {
        var s = await MohiniAPI.get('/api/status');
        this.connected = true;
        this.booting = false;
        this.lastError = '';
        ...
        console.warn('[Mohini] Status check failed:', e.message);
      }
    },

    async checkOnboarding() {
      if (localStorage.getItem('mohini-onboarded')) return;
      try {
        var config = await MohiniAPI.get('/api/config');
    ...
    async checkAuth() {
      try {
        ...
        if (e.message && (e.message.indexOf('Not authorized') >= 0 || e.message.indexOf('401') >= 0 || e.message.indexOf('Missing Authorization') >= 0 || e.message.indexOf('Unauthorized') >= 0)) {
          ...
          var saved = localStorage.getItem('mohini-api-key');
          if (saved) {
            // Saved key might be stale — clear it and show prompt
            MohiniAPI.setAuthToken('');
            localStorage.removeItem('mohini-api-key');
          }
    ...
    submitApiKey(key) {
      ...
      MohiniAPI.setAuthToken(key.trim());
      localStorage.setItem('mohini-api-key', key.trim());
      this.showAuthPrompt = false;
      this.refreshAgents();
    },
...
function app() {
  return {
    page: 'agents',
    themeMode: localStorage.getItem('mohini-theme-mode') || 'system',
    theme: (() => {
      var mode = localStorage.getItem('mohini-theme-mode') || 'system';
      if (mode === 'system') return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
      return mode;
    })(),
    sidebarCollapsed: localStorage.getItem('mohini-sidebar') === 'collapsed',
    ...
    init() {
      ...
      MohiniAPI.onConnectionChange(function(state) {
    ...
    setTheme(mode) {
      this.themeMode = mode;
      localStorage.setItem('mohini-theme-mode', mode);
      if (mode === 'system') {
    ...
    toggleSidebar() {
      ...
      localStorage.setItem('mohini-sidebar', this.sidebarCollapsed ? 'collapsed' : 'expanded');
    },
    ...
    async pollStatus() {
      ...
      this.wsConnected = MohiniAPI.isWsConnected();
    }
  };
}
