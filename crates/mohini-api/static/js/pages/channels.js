// Mohini Channels Page — LegacyImport-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async loadChannels() {
      ...
      this.loadError = '';
      try {
        var data = await MohiniAPI.get('/api/channels');
    ...
    async startQR() {
      ...
      try {
        var result = await MohiniAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          MohiniToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await MohiniAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            MohiniToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await MohiniAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await MohiniAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            MohiniToast.success(this.setupModal.display_name + ' activated!');
          } else {
            MohiniToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          MohiniToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
    ...
    async removeChannel() {
      ...
      var self = this;
      MohiniToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await MohiniAPI.delete('/api/channels/' + name + '/configure');
          MohiniToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await MohiniAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          MohiniToast.success(result.message);
        } else {
          MohiniToast.error(result.message);
        }
      } catch(e) {
        MohiniToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        MohiniToast.success('Copied to clipboard');
      } catch(e) {
        MohiniToast.error('Copy failed');
      }
    },
