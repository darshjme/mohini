// Mohini Comms Page — Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          MohiniAPI.get('/api/comms/topology'),
          MohiniAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      var self = this;
      var url = MohiniAPI.baseUrl + '/api/comms/events/stream';
      if (MohiniAPI.apiKey) url += '?token=' + encodeURIComponent(MohiniAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await MohiniAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await MohiniAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        MohiniToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        MohiniToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await MohiniAPI.post('/api/comms/task', body);
        MohiniToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        MohiniToast.error(e.message || 'Task failed');
      }
