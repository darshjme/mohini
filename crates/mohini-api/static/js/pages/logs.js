// Mohini Logs Page — Real-time log viewer (SSE streaming + polling fallback) + Audit Trail tab
'use strict';
...
function logsPage() {
  return {
    ...
    async fetchLogs() {
      ...
      try {
        var data = await MohiniAPI.get('/api/audit/recent?n=200');
    ...
    exportLogs: function() {
      ...
      a.href = url;
      a.download = 'mohini-logs-' + new Date().toISOString().slice(0, 10) + '.txt';
      a.click();
    ...
    async verifyChain() {
      try {
        var data = await MohiniAPI.get('/api/audit/verify');
        this.chainValid = data.valid === true;
        if (this.chainValid) {
          MohiniToast.success('Audit chain verified — ' + (data.entries || 0) + ' entries valid');
        } else {
          MohiniToast.error('Audit chain broken!');
        }
      } catch(e) {
        this.chainValid = false;
        MohiniToast.error('Chain verification failed: ' + e.message);
      }
    },
