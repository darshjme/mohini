// Mohini Scheduler Page — Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadJobs() {
      var data = await MohiniAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await MohiniAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        MohiniToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        MohiniToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await MohiniAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true };
        MohiniToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        MohiniToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await MohiniAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        MohiniToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        MohiniToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      MohiniToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await MohiniAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          MohiniToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          MohiniToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await MohiniAPI.post('/api/schedules/' + job.id + '/run', {});
        if (result.status === 'completed') {
          MohiniToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          MohiniToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        MohiniToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await MohiniAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        MohiniToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        MohiniToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      MohiniToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await MohiniAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          MohiniToast.success('Trigger deleted');
        } catch(e) {
          MohiniToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
