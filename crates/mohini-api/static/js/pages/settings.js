// Mohini Settings Page — Provider Hub, Model Catalog, Config, Tools + Security, Network, Migration tabs
'use strict';
  ...
  return {
    ...
    coreFeatures: [
      ...
      },
      {
        ...
        description: 'Agent-to-agent MMP connections use HMAC-SHA256 mutual authentication with nonce-based handshake and constant-time signature comparison (subtle crate).',
    ...
    configurableFeatures: [
      ...
      },
      {
        ...
        configHint: 'Set api_key in ~/.mohini/config.toml for remote access. Empty = localhost only.',
        valueKey: 'auth'
      }
    ],
    ...
    async loadSysInfo() {
      try {
        var ver = await MohiniAPI.get('/api/version');
        var status = await MohiniAPI.get('/api/status');
        this.sysInfo = {
          version: ver.version || '-',
    ...
    async loadUsage() {
      try {
        var data = await MohiniAPI.get('/api/usage');
    ...
    async loadTools() {
      try {
        var data = await MohiniAPI.get('/api/tools');
    ...
    async loadConfig() {
      try {
        this.config = await MohiniAPI.get('/api/config');
      } catch(e) { this.config = {}; }
    },

    async loadProviders() {
      try {
        var data = await MohiniAPI.get('/api/providers');
    ...
    async loadModels() {
      try {
        var data = await MohiniAPI.get('/api/models');
    ...
    async addCustomModel() {
      ...
      try {
        await MohiniAPI.post('/api/models/custom', {
          id: id,
    ...
    async deleteCustomModel(modelId) {
      ...
      try {
        await MohiniAPI.del('/api/models/custom/' + encodeURIComponent(modelId));
        MohiniToast.success('Model deleted');
        await this.loadModels();
      } catch(e) {
        MohiniToast.error('Failed to delete: ' + (e.message || 'Unknown error'));
      }
    },
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          MohiniAPI.get('/api/config/schema').catch(function() { return {}; }),
          MohiniAPI.get('/api/config')
        ]);
    ...
    async saveConfigField(section, field, value) {
      ...
      try {
        await MohiniAPI.post('/api/config/set', { path: path, value: value });
        this.configDirty[key] = false;
        MohiniToast.success('Saved ' + field);
      } catch(e) {
        MohiniToast.error('Failed to save: ' + e.message);
      }
    ...
    async saveProviderKey(provider) {
      ...
      if (!key || !key.trim()) { MohiniToast.error('Please enter an API key'); return; }
      try {
        await MohiniAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key.trim() });
        MohiniToast.success('API key saved for ' + provider.display_name);
        this.providerKeyInputs[provider.id] = '';
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        MohiniToast.error('Failed to save key: ' + e.message);
      }
    },

    async removeProviderKey(provider) {
      try {
        await MohiniAPI.del('/api/providers/' + encodeURIComponent(provider.id) + '/key');
        MohiniToast.success('API key removed for ' + provider.display_name);
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        MohiniToast.error('Failed to remove key: ' + e.message);
      }
    },
    ...
    async startCopilotOAuth() {
      ...
      try {
        var resp = await MohiniAPI.post('/api/providers/github-copilot/oauth/start', {});
        ...
        MohiniToast.error('Failed to start Copilot login: ' + e.message);
    ...
    pollCopilotOAuth() {
      ...
      setTimeout(async function() {
        ...
        try {
          var resp = await MohiniAPI.get('/api/providers/github-copilot/oauth/poll/' + self.copilotOAuth.pollId);
          if (resp.status === 'complete') {
            MohiniToast.success('GitHub Copilot authenticated successfully!');
            ...
            MohiniToast.error('Device code expired. Please try again.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else if (resp.status === 'denied') {
            MohiniToast.error('Access denied by user.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else {
            MohiniToast.error('OAuth error: ' + (resp.error || resp.status));
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          }
        } catch(e) {
          MohiniToast.error('Poll error: ' + e.message);
          self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
        }
    ...
    async testProvider(provider) {
      ...
      try {
        var result = await MohiniAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/test', {});
        this.providerTestResults[provider.id] = result;
        if (result.status === 'ok') {
          MohiniToast.success(provider.display_name + ' connected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          MohiniToast.error(provider.display_name + ': ' + (result.error || 'Connection failed'));
        }
      } catch(e) {
        this.providerTestResults[provider.id] = { status: 'error', error: e.message };
        MohiniToast.error('Test failed: ' + e.message);
      }
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { MohiniToast.error('Please enter a base URL'); return; }
      url = url.trim();
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        MohiniToast.error('URL must start with http:// or https://'); return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await MohiniAPI.put('/api/providers/' + encodeURIComponent(provider.id) + '/url', { base_url: url });
        if (result.reachable) {
          MohiniToast.success(provider.display_name + ' URL saved &mdash; reachable (' + (result.latency_ms || '?') + 'ms)');
        } else {
          MohiniToast.warning(provider.display_name + ' URL saved but not reachable');
        }
        await this.loadProviders();
      } catch(e) {
        MohiniToast.error('Failed to save URL: ' + e.message);
      }
    ...
    async addCustomProvider() {
      ...
      if (!name) { MohiniToast.error('Please enter a provider name'); return; }
      var url = this.customProviderUrl.trim();
      if (!url) { MohiniToast.error('Please enter a base URL'); return; }
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        MohiniToast.error('URL must start with http:// or https://'); return;
      }
      ...
      try {
        var result = await MohiniAPI.put('/api/providers/' + encodeURIComponent(name) + '/url', { base_url: url });
        if (this.customProviderKey.trim()) {
          await MohiniAPI.post('/api/providers/' + encodeURIComponent(name) + '/key', { key: this.customProviderKey.trim() });
        }
        this.customProviderName = '';
        this.customProviderUrl = '';
        this.customProviderKey = '';
        this.customProviderStatus = '';
        MohiniToast.success('Provider "' + name + '" added' + (result.reachable ? ' (reachable)' : ' (not reachable yet)'));
        await this.loadProviders();
      } catch(e) {
        this.customProviderStatus = 'Error: ' + (e.message || 'Failed');
        MohiniToast.error('Failed to add provider: ' + e.message);
      }
    ...
    async loadSecurity() {
      ...
      try {
        this.securityData = await MohiniAPI.get('/api/security');
      } catch(e) {
    ...
    async verifyAuditChain() {
      ...
      try {
        var res = await MohiniAPI.get('/api/audit/verify');
    ...
    async loadPeers() {
      ...
      this.peersLoadError = '';
      try {
        var data = await MohiniAPI.get('/api/peers');
    ...
    async autoDetect() {
      this.detecting = true;
      try {
        var data = await MohiniAPI.get('/api/migrate/detect');
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await MohiniAPI.post('/api/migrate/scan', { path: this.sourcePath });
        if (data.error) {
          MohiniToast.error('Scan error: ' + data.error);
          this.scanning = false;
          return;
        }
        ...
        MohiniToast.error('Scan failed: ' + e.message);
      }
      this.scanning = false;
    },
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        if (!target) target = '';
        var data = await MohiniAPI.post('/api/migrate', {
