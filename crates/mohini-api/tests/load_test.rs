//! Load & performance tests for the Mohini API.
//!
//! Measures throughput under concurrent access: agent spawning, API endpoint
//! latency, session management, and memory usage.
//!
//! Run: cargo test -p mohini-api --test load_test -- --nocapture

use axum::Router;
use mohini_api::middleware;
use mohini_api::routes::{self, AppState};
use mohini_kernel::MohiniKernel;
use mohini_types::config::{DefaultModelConfig, KernelConfig};
use std::sync::Arc;
...
async fn start_test_server() -> TestServer {
    ...
    let kernel = MohiniKernel::boot_with_config(config).expect("Kernel should boot");
...
async fn load_metrics_sustained() {
    ...
    for _ in 0..n {
        ...
        assert!(body.contains("mohini_agents_active"));
    }
