//! Channel bridge — connects channel adapters to the Mohini kernel.
//!
//! Defines `ChannelBridgeHandle` (implemented by mohini-api on the kernel) and
...
use dashmap::DashMap;
use mohini_types::message::ContentBlock;
use futures::StreamExt;
use mohini_types::agent::AgentId;
use mohini_types::config::{ChannelOverrides, DmPolicy, GroupPolicy, OutputFormat};
use std::sync::Arc;
...
///
/// Defined here to avoid circular deps (mohini-channels can't depend on mohini-kernel).
/// Implemented in mohini-api on the actual kernel.
#[async_trait]
...
fn channel_type_str(channel: &crate::types::ChannelType) -> &str {
    match channel {
        ...
        crate::types::ChannelType::IMessage => "imessage",
...
async fn dispatch_message(
    ...
    if router.has_broadcast(&message.sender.platform_id) {
        ...
        if !targets.is_empty() {
            ...
            match strategy {
                mohini_types::config::BroadcastStrategy::Parallel => {
                ...
                mohini_types::config::BroadcastStrategy::Sequential => {
                    for (name, maybe_id) in &targets {
                        if let Some(aid) = maybe_id {
    ...
    let agent_id = router.resolve(
        ...
        message.sender.mohini_user.as_deref(),
    );
...
async fn handle_command(
    ...
    match name {
        "start" => {
            ...
            let mut msg = "Welcome to Mohini! I connect you to AI agents.\n\nAvailable agents:\n"
                .to_string();
            if agents.is_empty() {
        ...
        "help" => "Mohini Bot Commands:\n\
             ...
             /peers - show MMP peer network status\n\
