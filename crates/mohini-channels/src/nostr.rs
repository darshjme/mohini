...
impl ChannelAdapter for NostrAdapter {
    ...
    {
        let pubkey = self.derive_pubkey();
        info!("Nostr adapter starting (pubkey: {}...)", mohini_types::truncate_str(&pubkey, 16));
                    ...
                    let should_reconnect = loop {
                        let msg = tokio::select! {
                            _ = relay_shutdown_rx.changed() => {
                                ...
                                // Send CLOSE
                                let close_msg = serde_json::json!(["CLOSE", "mohini-sub"]);
                                let _ = write.send(
                        ...
                        let channel_msg = ChannelMessage {
                            ...
                            sender: ChannelUser {
                                ...
                                display_name: format!(
                                    "{}...",
                                    mohini_types::truncate_str(&sender_pubkey, 8)
                                ),
                                mohini_user: None,
                            },
                            content: msg_content,
...
mod tests {
    ...
    fn test_nostr_build_subscription() {
        ...
        assert!(sub.contains("mohini-sub"));
