...
pub struct ChannelUser {
    ...
    /// Optional mapping to an Mohini user identity.
    pub mohini_user: Option<String>,
}
...
// Re-export policy/format types from mohini-types for convenience.
pub use mohini_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the Mohini kernel by converting
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = mohini_types::truncate_str(remaining, max_len).len();
