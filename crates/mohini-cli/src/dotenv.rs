//! Minimal `.env` file loader/saver for `~/.mohini/.env`.
//!
...
fn dotenv_mohini_home() -> Option<PathBuf> {
    if let Ok(home) = std::env::var("MOHINI_HOME") {
        return Some(PathBuf::from(home));
    }
    dirs::home_dir().map(|h| h.join(".mohini"))
}

/// Return the path to `~/.mohini/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    dotenv_mohini_home().map(|h| h.join(".env"))
}

/// Load `~/.mohini/.env` and `~/.mohini/secrets.env` into `std::env`.
///
...
/// Return the path to `~/.mohini/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    dotenv_mohini_home().map(|h| h.join("secrets.env"))
}
...
/// List key names (without values) from `~/.mohini/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content =
        String::from("# Mohini environment — managed by `mohini config set-key`\n");
