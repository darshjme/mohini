...
//!
//! Shown when `mohini` is run with no subcommand in a TTY.
...
fn is_first_run() -> bool {
    let of_home = if let Ok(h) = std::env::var("MOHINI_HOME") {
...
fn has_legacy_import() -> bool {
    // Quick check: does ~/.legacy_import exist?
    dirs::home_dir()
        .map(|h| h.join(".legacy_import").exists())
        .unwrap_or(false)
}
...
struct LauncherState {
    ...
    tick: usize,
    first_run: bool,
    legacy_import_detected: bool,
}

impl LauncherState {
    fn new() -> Self {
        let first_run = is_first_run();
        let legacy_import_detected = first_run && has_legacy_import();
...
fn draw(frame: &mut ratatui::Frame, state: &mut LauncherState) {
    ...
    let migration_hint_h: u16 = if state.first_run && state.legacy_import_detected {
        2
    } else {
        0
    };
    ...
    let chunks = Layout::vertical([
        ...
        Constraint::Length(migration_hint_h), // legacy_import migration hint (if any)
    ...
    // ── LegacyImport migration hint ─────────────────────────────────────────────
    if state.first_run && state.legacy_import_detected {
        let hint_lines = vec![
            Line::from(""),
            Line::from(vec![
                Span::styled("\u{2192} ", Style::default().fg(theme::BLUE)),
                Span::styled("Coming from LegacyImport? ", Style::default().fg(theme::BLUE)),
                Span::styled(
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ui::error_with_fix(
                        ...
                        "Build it: cargo build -p mohini-desktop",
                    );
                }
            }
        }
