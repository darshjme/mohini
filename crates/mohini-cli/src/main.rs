//! Mohini CLI — command-line interface for the Mohini Agent OS.
//!
//! When a daemon is running (`mohini start`), the CLI talks to it over HTTP.
...
use mohini_api::server::read_daemon_info;
use mohini_kernel::MohiniKernel;
use mohini_types::agent::{AgentId, AgentManifest};
...
\x1b[1;36mExamples:\x1b[0m
  mohini init                 Initialize config and data directories
  mohini start                Start the kernel daemon
  mohini tui                  Launch the interactive terminal dashboard
  mohini chat                 Quick chat with the default agent
  mohini agent new coder      Spawn a new agent from a template
  mohini models list          Browse available LLM models
  mohini add github           Install the GitHub integration
  mohini doctor               Run diagnostic health checks
  mohini channel setup        Interactive channel setup wizard
  mohini cron list            List scheduled jobs
  mohini uninstall            Completely remove Mohini from your system

\x1b[1;36mQuick Start:\x1b[0m
  1. mohini init              Set up config + API key
  2. mohini start             Launch the daemon
...
#[command(
    name = "mohini",
    version,
    about = "\u{1F40D} Mohini \u{2014} Open-source Agent Operating System",
    long_about = "\u{1F40D} Mohini \u{2014} Open-source Agent Operating System\n\n\
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize Mohini (create ~/.mohini/ and default config).
    Init {
        /// Quick mode: no prompts, just write config + .env (for CI/scripts).
        #[arg(long)]
        quick: bool,
    },
    /// Start the Mohini kernel daemon (API server + kernel).
    Start,
    ...
    /// Migrate from another agent framework to Mohini.
    Migrate(MigrateArgs),
    ...
    /// Completely uninstall Mohini from your system.
    Uninstall {
...
#[derive(Subcommand)]
enum SkillCommands {
    /// Install a skill from SkillHub or a local directory.
    Install {
...
enum ConfigCommands {
    ...
    /// Save an API key to ~/.mohini/.env (prompts interactively).
    SetKey {
...
/// Get the Mohini home directory, respecting MOHINI_HOME env var.
fn cli_mohini_home() -> std::path::PathBuf {
    if let Ok(home) = std::env::var("MOHINI_HOME") {
...
fn main() {
    // Load ~/.mohini/.env into process environment (system env takes priority).
    dotenv::load_dotenv();
    ...
    match resp {
        Ok(r) => {
            ...
            if status.is_server_error() {
                ui::error_with_fix(
                    ...
                    "Check daemon logs: ~/.mohini/tui.log",
                );
            }
            body
        }
        Err(e) => {
            ...
            if msg.contains("timed out") || msg.contains("Timeout") {
                ui::error_with_fix(
                    "Request timed out",
                    "The agent may be processing a complex request. Try again, or check `mohini status`",
                );
            } else if msg.contains("Connection refused") || msg.contains("connect") {
                ui::error_with_fix(
                    "Cannot connect to daemon",
                    "Is the daemon running? Start it with: mohini start",
                );
            } else {
                ui::error_with_fix(
                    &format!("Daemon communication error: {msg}"),
                    "Check `mohini status` or restart: mohini start",
                );
            }
            std::process::exit(1);
        }
...
fn cmd_init(quick: bool) {
    ...
    let mohini_dir = cli_mohini_home();

    // --- Ensure directories exist ---
    if !mohini_dir.exists() {
        std::fs::create_dir_all(&mohini_dir).unwrap_or_else(|e| {
            ui::error_with_fix(
                &format!("Failed to create {}", mohini_dir.display()),
                &format!("Check permissions on {}", home.display()),
            );
            eprintln!("  {e}");
            std::process::exit(1);
        });
        restrict_dir_permissions(&mohini_dir);
    }

    for sub in ["data", "agents"] {
        let dir = mohini_dir.join(sub);
        if !dir.exists() {
    ...
    bundled_agents::install_bundled_agents(&mohini_dir.join("agents"));

    if quick {
        cmd_init_quick(&mohini_dir);
    } else if !std::io::IsTerminal::is_terminal(&std::io::stdin())
        || !std::io::IsTerminal::is_terminal(&std::io::stdout())
    {
        ui::hint("Non-interactive terminal detected — running in quick mode");
        ui::hint("For the interactive wizard, run: mohini init (in a terminal)");
        cmd_init_quick(&mohini_dir);
    } else {
        cmd_init_interactive(&mohini_dir);
    }
}

/// Quick init: no prompts, auto-detect, write config + .env, print next steps.
fn cmd_init_quick(mohini_dir: &std::path::Path) {
    ui::banner();
    ui::blank();

    let (provider, api_key_env, model) = detect_best_provider();

    write_config_if_missing(mohini_dir, provider, model, api_key_env);

    ui::blank();
    ui::success("Mohini initialized (quick mode)");
    ui::kv("Provider", provider);
    ui::kv("Model", model);
    ui::blank();
    ui::next_steps(&[
        "Start the daemon:  mohini start",
        "Chat:              mohini chat",
    ]);
}

/// Interactive 5-step onboarding wizard (ratatui TUI).
fn cmd_init_interactive(mohini_dir: &std::path::Path) {
            ...
            match launch {
                ...
                LaunchChoice::Dashboard => {
                    if let Some(base) = find_daemon() {
                        ...
                        ui::error("Daemon is not running. Start it with: mohini start");
                    }
                }
                LaunchChoice::Chat => {
...
/// Launch the mohini-desktop Tauri app, connecting to the running daemon.
fn launch_desktop_app(_mohini_dir: &std::path::Path) {
    ...
    let desktop_bin = {
        ...
        #[cfg(windows)]
        let name = "mohini-desktop.exe";
        #[cfg(not(windows))]
        let name = "mohini-desktop";
    ...
    match desktop_bin {
        Some(ref path) if path.exists() => {
            ui::success("Launching Mohini Desktop...");
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ...
                    ui::hint("Try: mohini dashboard");
                }
            }
        }
        _ => {
            ui::error("Desktop app not found.");
            ui::hint("Install it with: cargo install mohini-desktop");
            ...
            if let Some(base) = find_daemon() {
                ...
                ui::hint("Daemon is not running. Start it with: mohini start");
...
fn write_config_if_missing(
    mohini_dir: &std::path::Path,
    provider: &str,
    model: &str,
    api_key_env: &str,
) {
    let config_path = mohini_dir.join("config.toml");
...
fn cmd_start(config: Option<PathBuf>) {
    if let Some(base) = find_daemon() {
        ui::error_with_fix(
            ...
            "Use `mohini status` to check it, or stop it first",
        );
        std::process::exit(1);
    }
    ...
    rt.block_on(async {
        let kernel = match MohiniKernel::boot(config.as_deref()) {
            Ok(k) => k,
            Err(e) => {
                boot_kernel_error(&e);
                std::process::exit(1);
        ...
        ui::kv("Model", &model);
        ui::blank();
        ui::hint("Open the dashboard in your browser, or run `mohini chat`");
        ui::hint("Press Ctrl+C to stop the daemon");
        ui::blank();

        if let Err(e) =
            mohini_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
        ...
        ui::blank();
        println!("  Mohini daemon stopped.");
    });
}

/// Read the api_key from ~/.mohini/config.toml (if any).
///
...
fn read_api_key() -> Option<String> {
    ...
    let config_path = cli_mohini_home().join("config.toml");
    ...
    // 2. Fall back to MOHINI_API_KEY env var
    if let Ok(key) = std::env::var("MOHINI_API_KEY") {
    ...
    match find_daemon() {
        ...
        None => {
            ui::warn_with_fix(
                ...
                "Is it running? Check with: mohini status",
            );
        }
    }
}
...
fn cmd_agent_spawn(config: Option<PathBuf>, manifest_path: PathBuf) {
    if !manifest_path.exists() {
        ui::error_with_fix(
            ...
            "Use `mohini agent new` to spawn from a template instead",
        );
        std::process::exit(1);
    }
    ...
    if let Some(base) = find_daemon() {
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                println!("  For persistent agents, use `mohini start` first.");
            }
            Err(e) => {
    ...
    match field {
        "model" => {
            if let Some(base) = find_daemon() {
                ...
                eprintln!("No running daemon found. Start one with: mohini start");
                std::process::exit(1);
            }
        }
        _ => {
...
fn cmd_agent_new(config: Option<PathBuf>, template_name: Option<String>) {
    ...
    if all_templates.is_empty() {
        ui::error_with_fix(
            ...
            "Run `mohini init` to set up the agents directory",
        );
        std::process::exit(1);
    }
        ...
        Some(ref name) => match all_templates.iter().find(|t| t.name == *name) {
            ...
            None => {
                ui::error_with_fix(
                    ...
                    "Run `mohini agent new` to see available templates",
                );
                std::process::exit(1);
            }
        },
...
fn spawn_template_agent(config: Option<PathBuf>, template: &templates::AgentTemplate) {
    if let Some(base) = find_daemon() {
        ...
        if let Some(id) = body["agent_id"].as_str() {
            ...
            ui::blank();
            ui::hint(&format!("Chat: mohini chat {}", template.name));
        } else {
            ui::error(&format!(
                "Failed to spawn: {}",
                body["error"].as_str().unwrap_or("Unknown error")
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                ui::hint("For persistent agents, use `mohini start` first");
            }
            Err(e) => {
...
fn cmd_status(config: Option<PathBuf>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("Mohini Daemon Status");
        ui::blank();
        ...
        ui::section("Mohini Status (in-process)");
        ui::blank();
        ...
        ui::blank();
        ui::hint("Run `mohini start` to launch the daemon");
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if let Some(_h) = &home {
        ...
        // --- Check 1: Mohini directory ---
        if mohini_dir.exists() {
            if !json {
                ui::check_ok(&format!("Mohini directory: {}", mohini_dir.display()));
            }
            checks.push(serde_json::json!({"check": "mohini_dir", "status": "ok", "path": mohini_dir.display().to_string()}));
        } else if repair {
            if !json {
                ui::check_fail("Mohini directory not found.");
            }
            let answer = prompt_input("    Create it now? [Y/n] ");
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                if std::fs::create_dir_all(&mohini_dir).is_ok() {
                    restrict_dir_permissions(&mohini_dir);
                    for sub in ["data", "agents"] {
                        let _ = std::fs::create_dir_all(mohini_dir.join(sub));
                    }
                    if !json {
                        ui::check_ok("Created Mohini directory");
                    }
                    repaired = true;
                } else {
                    if !json {
            ...
            checks.push(serde_json::json!({"check": "mohini_dir", "status": if repaired { "repaired" } else { "fail" }}));
        } else {
            if !json {
                ui::check_fail("Mohini directory not found. Run `mohini init` first.");
            }
            checks.push(serde_json::json!({"check": "mohini_dir", "status": "fail"}));
            all_ok = false;
        }

        // --- Check 2: .env file exists + permissions ---
        let env_path = mohini_dir.join(".env");
        if env_path.exists() {
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt;
            ...
            if !json {
                ui::check_warn(
                    ".env file not found (create with: mohini config set-key <provider>)",
                );
            }
            ...
            match toml::from_str::<toml::Value>(&config_content) {
                ...
                Err(e) => {
                    if !json {
                        ...
                        ui::hint("Fix with: mohini config edit");
                    }
...
"#
                ...
                let _ = std::fs::create_dir_all(&mohini_dir);
        ...
        let api_listen = {
            let cfg_path = mohini_dir.join("config.toml");
            if cfg_path.exists() {
                std::fs::read_to_string(&cfg_path)
                    .ok()
                    .and_then(|s| toml::from_str::<mohini_types::config::KernelConfig>(&s).ok())
                    .map(|c| c.api_listen)
        ...
        if let Some(ref base) = daemon_running {
            ...
            if !json {
                ui::check_warn("Daemon not running (start with `mohini start`)");
            }
        ...
        let daemon_json_path = mohini_dir.join("daemon.json");
        ...
        let db_path = mohini_dir.join("data").join("mohini.db");
        if db_path.exists() {
        ...
        #[cfg(unix)]
        {
            if let Ok(output) = std::process::Command::new("df")
                .args(["-m", &mohini_dir.display().to_string()])
                .output()
            {
        ...
        let agents_dir = mohini_dir.join("agents");
    ...
    if !any_key_set {
        if !json {
            ...
            ui::blank();
            ui::hint("Or run: mohini config set-key groq");
        }
        all_ok = false;
    }
    ...
    // --- Check 12: Config deserialization into KernelConfig ---
    {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<mohini_types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = mohini_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
                    ...
                    if !cfg.mcp_servers.is_empty() {
                        ...
                        for server in &cfg.mcp_servers {
                            ...
                            match &server.transport {
                                mohini_types::config::McpTransportEntry::Stdio {
                                    command,
                                    ..
                                } => {
                                    if command.is_empty() {
                                ...
                                mohini_types::config::McpTransportEntry::Sse { url } => {
                                    if url.is_empty() {
                                        if !json {
                                            ui::check_warn(&format!(
    ...
    // --- Check 13: Skill registry health ---
    {
        ...
        let skills_dir = cli_mohini_home().join("skills");
        let mut skill_reg = mohini_skills::registry::SkillRegistry::new(skills_dir.clone());
        ...
        for skill in &skills {
            if let Some(ref prompt) = skill.manifest.prompt_context {
                let warnings = mohini_skills::verify::SkillVerifier::scan_prompt_content(prompt);
                let has_critical = warnings.iter().any(|w| {
                    matches!(
                        w.severity,
                        mohini_skills::verify::WarningSeverity::Critical
                    )
                });
                if has_critical {
    ...
    // --- Check 14: Extension registry health ---
    {
        ...
        let mut ext_registry =
            mohini_extensions::registry::IntegrationRegistry::new(&mohini_dir);
    ...
    if json {
        ...
        println!();
        if all_ok {
            ui::success("All checks passed! Mohini is ready.");
            ui::hint("Start the daemon: mohini start");
        } else if repaired {
            ui::success("Repairs applied. Re-run `mohini doctor` to verify.");
        } else {
            ui::error("Some checks failed.");
            if !repair {
                ui::hint("Run `mohini doctor --repair` to attempt auto-fix");
            }
        }
    }
}
        ...
        match start_daemon_background() {
            ...
            Err(e) => {
                ui::error_with_fix(
                    ...
                    "Start it manually: mohini start",
                );
                std::process::exit(1);
            }
        }
...
fn cmd_completion(shell: clap_complete::Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "mohini", &mut std::io::stdout());
}
...
fn require_daemon(command: &str) -> String {
    find_daemon().unwrap_or_else(|| {
        ui::error_with_fix(
            &format!("`mohini {command}` requires a running daemon"),
            "Start the daemon: mohini start",
        );
        ui::hint("Or try `mohini chat` which works without a daemon");
        std::process::exit(1);
    })
}

fn boot_kernel(config: Option<PathBuf>) -> MohiniKernel {
    match MohiniKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
            boot_kernel_error(&e);
            std::process::exit(1);
...
fn cmd_migrate(args: MigrateArgs) {
    let source = match args.from {
        MigrateSourceArg::Openclaw => mohini_migrate::MigrateSource::LegacyImport,
        MigrateSourceArg::Langchain => mohini_migrate::MigrateSource::LangChain,
        MigrateSourceArg::Autogpt => mohini_migrate::MigrateSource::AutoGpt,
    };
    ...
    let source_dir = args.source_dir.unwrap_or_else(|| {
        ...
        match source {
            mohini_migrate::MigrateSource::LegacyImport => home.join(".legacy_import"),
            mohini_migrate::MigrateSource::LangChain => home.join(".langchain"),
            mohini_migrate::MigrateSource::AutoGpt => home.join("Auto-GPT"),
        }
    });

    let target_dir = cli_mohini_home();
    ...
    let options = mohini_migrate::MigrateOptions {
        source,
        source_dir,
        target_dir,
    ...
    match mohini_migrate::run_migration(&options) {
        Ok(report) => {
...
fn cmd_skill_install(source: &str) {
    ...
    if source_path.exists() && source_path.is_dir() {
        ...
        if !manifest_path.exists() {
            // Check if it's an LegacyImport skill
            if mohini_skills::legacy_compat::detect_legacy_import_skill(&source_path) {
                println!("Detected LegacyImport skill format. Converting...");
                match mohini_skills::legacy_compat::convert_legacy_import_skill(&source_path) {
                    Ok(manifest) => {
                        let dest = skills_dir.join(&manifest.skill.name);
                        // Copy skill directory
                        copy_dir_recursive(&source_path, &dest);
                        if let Err(e) = mohini_skills::legacy_compat::write_mohini_manifest(
                            &dest, &manifest,
                        ) {
                            eprintln!("Failed to write manifest: {e}");
                            std::process::exit(1);
                        }
                        println!("Installed LegacyImport skill: {}", manifest.skill.name);
                    }
                    Err(e) => {
                        eprintln!("Failed to convert LegacyImport skill: {e}");
                        std::process::exit(1);
                    }
                }
                return;
        ...
        let manifest: mohini_skills::SkillManifest =
        ...
        // Remote install from SkillHub
        println!("Installing {source} from SkillHub...");
        let rt = tokio::runtime::Runtime::new().unwrap();
        let client = mohini_skills::marketplace::MarketplaceClient::new(
            mohini_skills::marketplace::MarketplaceConfig::default(),
        );
...
fn cmd_skill_list() {
    ...
    let mut registry = mohini_skills::registry::SkillRegistry::new(skills_dir);
...
"#
    ...
    println!("  2. Test locally: mohini skill test");
    println!(
        "  3. Install: mohini skill install {}",
        skill_dir.display()
    );
}
...
fn cmd_channel_list() {
    ...
    if !config_path.exists() {
        println!("No configuration found. Run `mohini init` first.");
        return;
    }
    ...
    println!("\nUse `mohini channel setup <channel>` to configure a channel.");
}
    ...
    match channel.as_str() {
        "telegram" => {
            ...
            match dotenv::save_env_key("TELEGRAM_BOT_TOKEN", &token) {
                Ok(()) => ui::success("Token saved to ~/.mohini/.env"),
            ...
            for (key, val) in [
                ...
                if !val.is_empty() {
                    match dotenv::save_env_key(key, val) {
                        Ok(()) => ui::success(&format!("{key} saved to ~/.mohini/.env")),
        ...
        "email" => {
            ...
            if !password.is_empty() {
                match dotenv::save_env_key("EMAIL_PASSWORD", &password) {
                    Ok(()) => ui::success("Password saved to ~/.mohini/.env"),
                    Err(_) => println!("    export EMAIL_PASSWORD=your_app_password"),
                }
            } else {
                ui::hint("Set later: mohini config set-key email (or export EMAIL_PASSWORD=...)");
            }
        ...
        "signal" => {
            ...
            if !phone.is_empty() {
                match dotenv::save_env_key("SIGNAL_PHONE", &phone) {
                    Ok(()) => ui::success("Phone saved to ~/.mohini/.env"),
        ...
        "matrix" => {
            ...
            println!("     (e.g., register @mohini-bot:matrix.org)");
            println!("  2. Obtain an access token:");
            println!("     curl -X POST https://matrix.org/_matrix/client/r0/login \\");
            println!("       -d '{{\"type\":\"m.login.password\",\"user\":\"mohini-bot\",\"password\":\"...\"}}'");
...
fn maybe_write_channel_config(channel: &str, config_block: &str) {
    ...
    if !config_path.exists() {
        ui::hint("No config.toml found. Run `mohini init` first.");
        return;
    }
...
fn cmd_channel_test(channel: &str) {
    if let Some(base) = find_daemon() {
        ...
        eprintln!("Channel test requires a running daemon. Start with: mohini start");
        std::process::exit(1);
    }
}
...
fn cmd_channel_toggle(channel: &str, enable: bool) {
    ...
    if let Some(base) = find_daemon() {
        ...
        println!("Edit ~/.mohini/config.toml to persist this change.");
    }
}
...
fn cmd_hand_install(path: &str) {
    ...
    println!("Use `mohini hand activate {}` to start it.", body["id"].as_str().unwrap_or("?"));
}
...
fn cmd_hand_list() {
    ...
    if let Some(arr) = Some(&arr_val) {
        ...
        println!("\nUse `mohini hand activate <id>` to activate a hand.");
    }
}
...
/// Spawn `mohini start` as a detached background process.
///
...
fn cmd_config_show() {
    ...
    if !config_path.exists() {
        ...
        println!("Run `mohini init` to create one.");
        return;
    }
...
fn cmd_config_get(key: &str) {
    ...
    if !config_path.exists() {
        ui::error_with_fix("No config file found", "Run `mohini init` first");
        std::process::exit(1);
    }
    ...
    let table: toml::Value = toml::from_str(&content).unwrap_or_else(|e| {
        ui::error_with_fix(
            ...
            "Fix your config.toml syntax, or run `mohini config edit`",
        );
        std::process::exit(1);
    });
...
fn cmd_config_set_key(provider: &str) {
    ...
    match dotenv::save_env_key(&env_var, &key) {
        Ok(()) => {
            ui::success(&format!("Saved {env_var} to ~/.mohini/.env"));
            // Test the key
            print!("  Testing key... ");
...
fn cmd_config_delete_key(provider: &str) {
    ...
    match dotenv::remove_env_key(&env_var) {
        Ok(()) => ui::success(&format!("Removed {env_var} from ~/.mohini/.env")),
        Err(e) => {
...
fn cmd_config_test_key(provider: &str) {
    ...
    if std::env::var(&env_var).is_err() {
        ...
        ui::hint(&format!("Set it: mohini config set-key {provider}"));
        std::process::exit(1);
    }
    ...
    if test_api_key(provider, &env_var) {
        ...
        ui::hint(&format!("Update key: mohini config set-key {provider}"));
        std::process::exit(1);
    }
}

// ---------------------------------------------------------------------------
// Quick chat (LegacyImport alias)
// ---------------------------------------------------------------------------
...
pub(crate) fn mohini_home() -> PathBuf {
    if let Ok(home) = std::env::var("MOHINI_HOME") {
...
// ---------------------------------------------------------------------------
// Integration commands (mohini add/remove/integrations)
// ---------------------------------------------------------------------------

fn cmd_integration_add(name: &str, key: Option<&str>) {
    let home = mohini_home();
    let mut registry = mohini_extensions::registry::IntegrationRegistry::new(&home);
    ...
    let vault = if vault_path.exists() {
        let mut v = mohini_extensions::vault::CredentialVault::new(vault_path);
        if v.unlock().is_ok() {
            Some(v)
        } else {
            None
    ...
    let mut resolver =
        mohini_extensions::credentials::CredentialResolver::new(vault, Some(&dotenv_path))
    ...
    match mohini_extensions::installer::install_integration(
        &mut registry,
        &mut resolver,
        name,
        &provided_keys,
        ...
        Ok(result) => {
            match &result.status {
                mohini_extensions::IntegrationStatus::Ready => {
                    ui::success(&result.message);
                }
                mohini_extensions::IntegrationStatus::Setup => {
                    println!("{}", result.message.yellow());
                    println!("\nTo add credentials:");
                    for env in &template.required_env {
                        if env.is_secret {
                            println!("  mohini vault set {}  # {}", env.name, env.help);
...
fn cmd_integration_remove(name: &str) {
    ...
    match mohini_extensions::installer::remove_integration(&mut registry, name) {
        Ok(msg) => {
            ui::success(&msg);
            // Hot-reload daemon
            if let Some(base_url) = find_daemon() {
...
fn cmd_integrations_list(query: Option<&str>) {
    ...
    let dotenv_path = home.join(".env");
    let resolver =
        mohini_extensions::credentials::CredentialResolver::new(None, Some(&dotenv_path));

    let entries = if let Some(q) = query {
        mohini_extensions::installer::search_integrations(&registry, q)
    } else {
        mohini_extensions::installer::list_integrations(&registry, &resolver)
    };
    ...
    let mut by_category: std::collections::BTreeMap<
        String,
        Vec<&mohini_extensions::installer::IntegrationListEntry>,
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let status_badge = match &item.status {
                mohini_extensions::IntegrationStatus::Ready => "[Ready]".green().to_string(),
                mohini_extensions::IntegrationStatus::Setup => "[Setup]".yellow().to_string(),
                mohini_extensions::IntegrationStatus::Available => {
                    "[Available]".dimmed().to_string()
                }
                mohini_extensions::IntegrationStatus::Error(msg) => {
                    format!("[Error: {msg}]").red().to_string()
                }
                mohini_extensions::IntegrationStatus::Disabled => {
    ...
    println!("  Use `mohini add <name>` to install an integration.");
}

// ---------------------------------------------------------------------------
// Vault commands (mohini vault init/set/list/remove)
// ---------------------------------------------------------------------------

fn cmd_vault_init() {
    let home = mohini_home();
    let vault_path = home.join("vault.enc");
    let mut vault = mohini_extensions::vault::CredentialVault::new(vault_path);
...
fn cmd_vault_set(key: &str) {
    ...
    if !vault.exists() {
        ui::error("Vault not initialized. Run: mohini vault init");
        std::process::exit(1);
    }
...
fn cmd_vault_list() {
    ...
    if !vault.exists() {
        println!("Vault not initialized. Run: mohini vault init");
        return;
    }
...
// ---------------------------------------------------------------------------
// Scaffold commands (mohini new skill/integration)
// ---------------------------------------------------------------------------
...
fn cmd_scaffold(kind: ScaffoldKind) {
    ...
    let result = match kind {
        ScaffoldKind::Skill => {
            mohini_extensions::installer::scaffold_skill(&cwd.join("my-skill"))
        }
        ScaffoldKind::Integration => {
            mohini_extensions::installer::scaffold_integration(&cwd.join("my-integration"))
        }
    };
    match result {
...
fn cmd_models_list(provider_filter: Option<&str>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        let catalog = mohini_runtime::model_catalog::ModelCatalog::new();
...
fn pick_model() -> String {
    ...
    let mut by_provider: std::collections::BTreeMap<
        String,
        Vec<&mohini_types::model_catalog::ModelCatalogEntry>,
    > = std::collections::BTreeMap::new();
    for m in models {
...
fn cmd_logs(lines: usize, follow: bool) {
    let log_path = cli_mohini_home().join("tui.log");
...
fn cmd_health(json: bool) {
    match find_daemon() {
        ...
        None => {
            ...
            ui::hint("Start it with: mohini start");
            std::process::exit(1);
        }
    }
}
...
fn cmd_security_status(json: bool) {
    ...
    ui::kv("Wire protocol", "MMP HMAC-SHA256 mutual auth");
...
fn cmd_devices_pair() {
    ...
    if let Some(qr) = body["qr_data"].as_str() {
        ...
        println!("  Scan this QR code with the Mohini mobile app:");
        ui::blank();
        println!("  {qr}");
        ui::blank();
...
fn cmd_system_info(json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::kv("Version", env!("CARGO_PKG_VERSION"));
        ui::kv_warn("Daemon", "NOT RUNNING");
        ui::hint("Start with: mohini start");
    }
}
...
fn cmd_system_version(json: bool) {
    ...
    println!("mohini {}", env!("CARGO_PKG_VERSION"));
}
...
fn cmd_reset(confirm: bool) {
    ...
    if !confirm {
        println!("  This will delete all data in {}", mohini_dir.display());
    ...
    match std::fs::remove_dir_all(&mohini_dir) {
        Ok(()) => ui::success(&format!("Removed {}", mohini_dir.display())),
        Err(e) => {
            ui::error(&format!("Failed to remove {}: {e}", mohini_dir.display()));
            std::process::exit(1);
        }
    }
}
...
fn cmd_uninstall(confirm: bool, keep_config: bool) {
    ...
    println!(
        "  {}",
        "This will completely uninstall Mohini from your system."
            .bold()
            .red()
    );
    println!();
    if mohini_dir.exists() {
        if keep_config {
            ...
            println!("  • Remove {}", mohini_dir.display());
        }
    }
    ...
    if find_daemon().is_some() {
        ...
        if find_daemon().is_some() {
            if let Some(info) = read_daemon_info(&mohini_dir) {
                force_kill_pid(info.pid);
                let _ = std::fs::remove_file(mohini_dir.join("daemon.json"));
            }
        }
    }
    ...
    // Step 6: Remove ~/.mohini/ data
    if mohini_dir.exists() {
        if keep_config {
            remove_dir_except_config(&mohini_dir);
    ...
    println!();
    ui::success("Mohini has been uninstalled. Goodbye!");
}
...
fn remove_autostart_entries(home: &std::path::Path) {
    ...
    #[cfg(target_os = "macos")]
    {
        let plist = home.join("Library/LaunchAgents/ai.mohini.desktop.plist");
        if plist.exists() {
            // Unload first
    ...
    #[cfg(target_os = "linux")]
    {
        let desktop_file = home.join(".config/autostart/Mohini.desktop");
        ...
        let service_file = home.join(".config/systemd/user/mohini.service");
        if service_file.exists() {
            let _ = std::process::Command::new("systemctl")
                .args(["--user", "disable", "--now", "mohini.service"])
                .output();
...
/// Remove lines from shell config files that add mohini to PATH.
#[allow(unused_variables)]
fn clean_path_entries(home: &std::path::Path, mohini_dir: &str) {
    #[cfg(not(windows))]
    {
        let shell_files = [
            home.join(".bashrc"),
        ...
        for path in &shell_files {
            ...
            let filtered: Vec<&str> = content
                .lines()
                .filter(|line| !is_mohini_path_line(line, mohini_dir))
                .collect();
    ...
    #[cfg(windows)]
    {
        // Read User PATH via PowerShell, filter out mohini entries, write back
        ...
        if let Ok(out) = output {
            if out.status.success() {
                ...
                if !current.is_empty() {
                    let dir_lower = mohini_dir.to_lowercase();
                    let filtered: Vec<&str> = current
                        .split(';')
                        .filter(|entry| {
                            let e = entry.trim().to_lowercase();
                            !e.is_empty() && !e.contains("mohini") && !e.contains(&dir_lower)
                        })
                        .collect();
...
/// Returns true if a shell config line is an mohini PATH export.
/// Must match BOTH an mohini reference AND a PATH-setting pattern.
#[cfg(any(not(windows), test))]
fn is_mohini_path_line(line: &str, mohini_dir: &str) -> bool {
    let lower = line.to_lowercase();
    let has_mohini = lower.contains("mohini") || lower.contains(&mohini_dir.to_lowercase());
    if !has_mohini {
        return false;
    }
...
/// Remove everything in ~/.mohini/ except config files.
fn remove_dir_except_config(mohini_dir: &std::path::Path) {
    let keep = ["config.toml", ".env", "secrets.env"];
    let Ok(entries) = std::fs::read_dir(mohini_dir) else {
        return;
    };
    for entry in entries.flatten() {
...
mod tests {
    ...
    fn test_doctor_skill_registry_loads_bundled() {
        let skills_dir = std::env::temp_dir().join("mohini-doctor-test-skills");
        let mut skill_reg = mohini_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn test_doctor_extension_registry_loads_bundled() {
        let tmp = std::env::temp_dir().join("mohini-doctor-test-ext");
        let _ = std::fs::create_dir_all(&tmp);
        let mut ext_reg = mohini_extensions::registry::IntegrationRegistry::new(&tmp);
    ...
    fn test_doctor_config_deser_default() {
        ...
        let config = mohini_types::config::KernelConfig::default();
        let toml_str = toml::to_string_pretty(&config).unwrap();
        let parsed: mohini_types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
"#;
        let config: mohini_types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            mohini_types::config::ExecSecurityMode::Allowlist
        );
...
"#;
        ...
        match &config.mcp_servers[0].transport {
            mohini_types::config::McpTransportEntry::Stdio { command, args } => {
    ...
    fn test_doctor_skill_injection_scan_clean() {
        ...
        let warnings = mohini_skills::verify::SkillVerifier::scan_prompt_content(clean_content);
    ...
    fn test_doctor_hook_event_variants() {
        ...
        use mohini_types::agent::HookEvent;
        let events = [
    ...
    fn test_uninstall_path_line_filter() {
        use super::is_mohini_path_line;
        let dir = "/home/user/.mohini/bin";

        // Should match: mohini PATH exports
        assert!(is_mohini_path_line(
            r#"export PATH="$HOME/.mohini/bin:$PATH""#,
            dir
        ));
        assert!(is_mohini_path_line(
            r#"export PATH="/home/user/.mohini/bin:$PATH""#,
            dir
        ));
        assert!(is_mohini_path_line(
            "set -gx PATH $HOME/.mohini/bin $PATH",
            dir
        ));
        assert!(is_mohini_path_line(
            "fish_add_path $HOME/.mohini/bin",
            dir
        ));
        ...
        // Should NOT match: mohini lines that aren't PATH-related
        assert!(!is_mohini_path_line("# mohini config", dir));
        assert!(!is_mohini_path_line("alias of=mohini", dir));
    }
}
