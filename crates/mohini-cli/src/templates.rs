...
/// 2. `~/.mohini/agents/` (installed templates)
/// 3. `MOHINI_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    // Installed templates (respects MOHINI_HOME)
    let of_home = if let Ok(h) = std::env::var("MOHINI_HOME") {
        PathBuf::from(h)
    } else if let Some(home) = dirs::home_dir() {
        home.join(".mohini")
    } else {
        std::env::temp_dir().join(".mohini")
    };
    {
    ...
    if let Ok(env_dir) = std::env::var("MOHINI_AGENTS_DIR") {
