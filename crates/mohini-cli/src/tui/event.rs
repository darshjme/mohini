...
use mohini_runtime::agent_loop::AgentLoopResult;
...
use super::screens::{
    ...
    skills::{SkillHubResult, McpServerInfo, SkillInfo},
...
pub enum AppEvent {
    ...
    /// SkillHub results loaded.
    SkillHubLoaded(Vec<SkillHubResult>),
    /// Skill installed.
                ...
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(data) {
                    ...
                    if json.get("done").and_then(|d| d.as_bool()) == Some(true) {
                        ...
                        let _ = tx.send(AppEvent::Stream(StreamEvent::ContentComplete {
                            ...
                            usage: mohini_types::message::TokenUsage {
...
pub fn spawn_fetch_agent_skills(backend: BackendRef, agent_id: String, tx: mpsc::Sender<AppEvent>) {
    std::thread::spawn(move || match backend {
        ...
        BackendRef::InProcess(kernel) => {
            if let Ok(uuid) = uuid::Uuid::parse_str(&agent_id) {
                let aid = mohini_types::agent::AgentId(uuid);
                let assigned = kernel
                    .registry
                    .get(aid)
...
pub fn spawn_search_skillhub(backend: BackendRef, query: String, tx: mpsc::Sender<AppEvent>) {
    std::thread::spawn(move || match backend {
        BackendRef::Daemon(base_url) => {
            let client = daemon_client();
            let encoded: String = query
                ...
                .collect();
            let url = format!("{base_url}/api/skillhub/search?q={encoded}");
            if let Ok(resp) = client.get(&url).send() {
                if let Ok(body) = resp.json::<serde_json::Value>() {
                    let results = parse_skillhub_results(&body);
                    let _ = tx.send(AppEvent::SkillHubLoaded(results));
                }
            }
        }
        BackendRef::InProcess(_) => {
            let _ = tx.send(AppEvent::SkillHubLoaded(Vec::new()));
        }
    });
}

/// Browse SkillHub marketplace.
pub fn spawn_browse_skillhub(backend: BackendRef, sort: String, tx: mpsc::Sender<AppEvent>) {
    std::thread::spawn(move || match backend {
        BackendRef::Daemon(base_url) => {
            let client = daemon_client();
            let url = format!("{base_url}/api/skillhub/browse?sort={sort}");
            if let Ok(resp) = client.get(&url).send() {
                if let Ok(body) = resp.json::<serde_json::Value>() {
                    let results = parse_skillhub_results(&body);
                    let _ = tx.send(AppEvent::SkillHubLoaded(results));
...
fn parse_skillhub_results(body: &serde_json::Value) -> Vec<SkillHubResult> {
...
pub fn spawn_install_skill(backend: BackendRef, slug: String, tx: mpsc::Sender<AppEvent>) {
    std::thread::spawn(move || match backend {
        BackendRef::Daemon(base_url) => {
            let client = daemon_client();
            match client
                .post(format!("{base_url}/api/skillhub/install"))
