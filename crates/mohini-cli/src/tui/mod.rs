//! Ratatui TUI for Mohini interactive mode.
//!
...
enum Backend {
    ...
    InProcess { kernel: Arc<MohiniKernel> },
    None,
}
...
impl App {
    ...
    fn handle_event(&mut self, ev: AppEvent) {
        match ev {
            ...
            AppEvent::SkillHubLoaded(results) => {
                self.skills.skillhub_results = results;
                if !self.skills.skillhub_results.is_empty() {
                    self.skills.skillhub_list.select(Some(0));
                }
                self.skills.loading = false;
            }
    ...
    fn handle_stream_done(
        &mut self,
        result: Result<mohini_runtime::agent_loop::AgentLoopResult, String>,
    ) {
    ...
    fn handle_kernel_ready(&mut self, kernel: Arc<MohiniKernel>) {
    ...
    fn handle_skills_action(&mut self, action: skills::SkillsAction) {
        match action {
            ...
            skills::SkillsAction::SearchSkillHub(query) => {
                if let Some(backend) = self.backend.to_ref() {
                    self.skills.loading = true;
                    event::spawn_search_skillhub(backend, query, self.event_tx.clone());
                }
            }
            skills::SkillsAction::BrowseSkillHub(sort) => {
                if let Some(backend) = self.backend.to_ref() {
                    self.skills.loading = true;
                    event::spawn_browse_skillhub(backend, sort, self.event_tx.clone());
                }
            }
