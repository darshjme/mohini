...
//!
//! Launched by `mohini init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use mohini_runtime::model_catalog::ModelCatalog;
use mohini_types::model_catalog::ModelTier;
...
struct State {
    ...
    legacy_import_path: Option<PathBuf>,
    legacy_import_scan: Option<mohini_migrate::legacy_import::ScanResult>,
    migration_report: Option<mohini_migrate::report::MigrationReport>,
...
impl State {
    ...
    fn build_provider_order(&mut self) {
        ...
        for (i, p) in PROVIDERS.iter().enumerate() {
            let detected = if p.name == "claude-code" {
                mohini_runtime::drivers::claude_code::claude_code_available()
            } else {
    ...
    fn is_provider_detected(&self, prov_idx: usize) -> bool {
        ...
        if p.name == "claude-code" {
            return mohini_runtime::drivers::claude_code::claude_code_available();
        }
        (!p.env_var.is_empty() && std::env::var(p.env_var).is_ok())
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<mohini_migrate::report::MigrationReport, String>>();
    ...
    let result = loop {
        ...
        if state.step == Step::Migration && state.migration_phase == MigrationPhase::Detecting {
            match mohini_migrate::legacy_import::detect_openclaw_home() {
                None => {
                    // No LegacyImport found — skip migration entirely
                ...
                Some(path) => {
                    ...
                    let has_content = scan.has_config
                        ...
                        || scan.has_memory;
                    if has_content {
                        state.legacy_import_path = Some(path);
                        state.legacy_import_scan = Some(scan);
            ...
            if let Ok(result) = migrate_rx.try_recv() {
                match result {
                    Ok(report) => {
                        ...
                        if let Some(scan) = &state.legacy_import_scan {
...
fn handle_migration_key(
    state: &mut State,
    code: KeyCode,
    migrate_tx: &std::sync::mpsc::Sender<Result<mohini_migrate::report::MigrationReport, String>>,
) {
        ...
        MigrationPhase::Offer => match code {
            ...
            KeyCode::Enter => {
                ...
                if yes {
                    ...
                    let source_dir = state.legacy_import_path.clone().unwrap_or_default();
                    let target_dir = if let Ok(h) = std::env::var("MOHINI_HOME") {
                        PathBuf::from(h)
                    } else {
                        dirs::home_dir().unwrap_or_else(|| PathBuf::from(".")).join(".mohini")
                    };
                    let tx = migrate_tx.clone();
                    std::thread::spawn(move || {
                        let options = mohini_migrate::MigrateOptions {
                            source: mohini_migrate::MigrateSource::LegacyImport,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result =
                            mohini_migrate::run_migration(&options).map_err(|e| format!("{e}"));
...
fn save_config(state: &mut State) {
    ...
    let mohini_dir = if let Ok(h) = std::env::var("MOHINI_HOME") {
        PathBuf::from(h)
    } else {
        match dirs::home_dir() {
            Some(h) => h.join(".mohini"),
            None => {
    ...
    let _ = std::fs::create_dir_all(mohini_dir.join("agents"));
    let _ = std::fs::create_dir_all(mohini_dir.join("data"));
    crate::restrict_dir_permissions(&mohini_dir);
...
/// Check if the `mohini-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "Mohini Init  Step X of 7"
...
fn draw_migration_offer(f: &mut Frame, area: Rect, state: &mut State) {
    let scan = match &state.legacy_import_scan {
        Some(s) => s,
        None => return,
    };
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  LegacyImport Installation Detected",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),
...
fn draw_migration_running(f: &mut Frame, area: Rect, state: &State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![
            Span::raw("  "),
            Span::styled(spinner, Style::default().fg(theme::ACCENT)),
            Span::raw(" Migrating from LegacyImport..."),
        ])),
        chunks[1],
    );
}
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use mohini_migrate::report::ItemKind;
...
fn draw_routing_pick(f: &mut Frame, area: Rect, state: &mut State, tier: usize) {
        ...
        .enumerate()
    {
        if t == tier {
            ...
            let display = mohini_types::truncate_str(short, 14);
            summary_spans.push(Span::styled(
                format!("{name}:{display}"),
                Style::default().fg(*c),
            ));
