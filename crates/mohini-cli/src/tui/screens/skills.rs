//! Skills screen: installed skills, SkillHub marketplace, MCP servers.
...
pub struct SkillsState {
    ...
    pub skillhub_results: Vec<SkillHubResult>,
    pub mcp_servers: Vec<McpServerInfo>,
    pub installed_list: ListState,
    pub skillhub_list: ListState,
    pub mcp_list: ListState,
    pub search_buf: String,
    pub search_mode: bool,
    pub sort: SkillHubSort,
    pub loading: bool,
    pub tick: usize,
...
impl SkillsState {
    pub fn new() -> Self {
        Self {
            ...
            installed: Vec::new(),
            skillhub_results: Vec::new(),
            mcp_servers: Vec::new(),
            installed_list: ListState::default(),
            skillhub_list: ListState::default(),
            mcp_list: ListState::default(),
            search_buf: String::new(),
            search_mode: false,
            sort: SkillHubSort::Trending,
            loading: false,
            tick: 0,
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> SkillsAction {
        ...
        if !self.search_mode {
            match key.code {
                ...
                KeyCode::Char('2') => {
                    self.sub = SkillsSub::SkillHub;
                    return SkillsAction::BrowseSkillHub(self.sort.label().to_string());
                }
                KeyCode::Char('3') => {
        ...
        match self.sub {
            ...
            SkillsSub::SkillHub => self.handle_skillhub(key),
    ...
    fn handle_skillhub(&mut self, key: KeyEvent) -> SkillsAction {
        if self.search_mode {
            match key.code {
                KeyCode::Esc => {
                ...
                KeyCode::Enter => {
                    ...
                    if !self.search_buf.is_empty() {
                        return SkillsAction::SearchSkillHub(self.search_buf.clone());
                    }
                }
                KeyCode::Backspace => {
                    self.search_buf.pop();
        ...
        let total = self.skillhub_results.len();
        match key.code {
            KeyCode::Up | KeyCode::Char('k') => {
                if total > 0 {
                    let i = self.skillhub_list.selected().unwrap_or(0);
                    let next = if i == 0 { total - 1 } else { i - 1 };
                    self.skillhub_list.select(Some(next));
                }
            }
            KeyCode::Down | KeyCode::Char('j') => {
                if total > 0 {
            ...
            KeyCode::Char('i') => {
                if let Some(sel) = self.skillhub_list.selected() {
                    if sel < self.skillhub_results.len() {
                        return SkillsAction::InstallSkill(self.skillhub_results[sel].slug.clone());
                    }
                }
            }
            KeyCode::Char('/') => {
...
pub fn draw(f: &mut Frame, area: Rect, state: &mut SkillsState) {
    ...
    match state.sub {
        ...
        SkillsSub::SkillHub => draw_skillhub(f, chunks[2], state),
    ...
    if state.loading {
        ...
        f.render_widget(
            Paragraph::new(Span::styled(
                "  No skills installed. Press [2] to browse SkillHub.",
                theme::dim_style(),
            )),
            chunks[1],
        );
        ...
        let items: Vec<ListItem> = state
            ...
            .map(|s| {
                ...
                let source_style = match s.source.as_str() {
                    "skillhub" => Style::default().fg(theme::ACCENT),
...
fn draw_skillhub(f: &mut Frame, area: Rect, state: &mut SkillsState) {
    let chunks = Layout::vertical([
    ...
    if state.loading {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled(format!("  {spinner} "), Style::default().fg(theme::CYAN)),
                Span::styled("Searching SkillHub\u{2026}", theme::dim_style()),
            ])),
            chunks[1],
        );
    } else if state.skillhub_results.is_empty() {
        f.render_widget(
            Paragraph::new(Span::styled(
        ...
        let list = List::new(items)
            .highlight_style(theme::selected_style())
            .highlight_symbol("> ");
        f.render_stateful_widget(list, chunks[1], &mut state.skillhub_list);
    }
...
fn truncate(s: &str, max: usize) -> String {
    if s.len() <= max {
        ...
        format!("{}\u{2026}", mohini_types::truncate_str(s, max.saturating_sub(1)))
    }
}
