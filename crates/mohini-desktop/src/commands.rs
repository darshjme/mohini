...
use mohini_kernel::config::mohini_home;
...
/// `~/.mohini/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = mohini_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.mohini/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = mohini_home().join("skills");
...
/// Open the Mohini config directory (`~/.mohini/`) in the OS file manager.
#[tauri::command]
...
/// Open the Mohini logs directory (`~/.mohini/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = mohini_home().join("logs");
