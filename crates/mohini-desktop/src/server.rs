...
//!
//! Boots the Mohini kernel, binds to a random localhost port, and runs the
//! API server on a background thread with its own tokio runtime.

use mohini_api::server::build_router;
...
impl ServerHandle {
    ...
    pub fn shutdown(mut self) {
            ...
            .is_ok()
        {
            ...
            info!("Mohini embedded server stopped");
        }
    }
}
...
pub fn start_server() -> Result<ServerHandle, Box<dyn std::error::Error>> {
    ...
    let kernel = MohiniKernel::boot(None)?;
    ...
    info!("Mohini embedded server bound to http://127.0.0.1:{port}");
...
async fn run_embedded_server(
    ...
    info!("Mohini embedded server listening on http://{listen_addr}");
