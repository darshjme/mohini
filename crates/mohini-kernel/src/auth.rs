...
//! to Mohini users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use mohini_types::agent::UserId;
use mohini_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their Mohini user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the Mohini UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> MohiniResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| MohiniError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(MohiniError::AuthDenied(format!(
