...
use mohini_memory::usage::{ModelUsage, UsageRecord, UsageStore, UsageSummary};
use mohini_types::agent::{AgentId, ResourceQuota};
...
impl MeteringEngine {
    ...
    pub fn check_quota(&self, agent_id: AgentId, quota: &ResourceQuota) -> MohiniResult<()> {
        // Hourly check
        if quota.max_cost_per_hour_usd > 0.0 {
            ...
            if hourly_cost >= quota.max_cost_per_hour_usd {
                return Err(MohiniError::QuotaExceeded(format!(
    ...
    pub fn budget_status(&self, budget: &mohini_types::config::BudgetConfig) -> BudgetStatus {
    ...
    pub fn get_summary(&self, agent_id: Option<AgentId>) -> MohiniResult<UsageSummary> {
        self.store.query_summary(agent_id)
    }

    /// Get usage grouped by model.
    pub fn get_by_model(&self) -> MohiniResult<Vec<ModelUsage>> {
    ...
    pub fn estimate_cost_with_catalog(
        catalog: &mohini_runtime::model_catalog::ModelCatalog,
        model: &str,
        input_tokens: u64,
        output_tokens: u64,
    ) -> f64 {
    ...
    pub fn cleanup(&self, days: u32) -> MohiniResult<usize> {
