...
use dashmap::DashMap;
use mohini_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> MohiniResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(MohiniError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> MohiniResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: mohini_types::agent::SessionId,
    ) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: mohini_types::agent::AgentIdentity,
    ) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_fallback_models(
        &self,
        id: AgentId,
        fallback_models: Vec<mohini_types::agent::FallbackModel>,
    ) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_skills(&self, id: AgentId, skills: Vec<String>) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_mcp_servers(&self, id: AgentId, servers: Vec<String>) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> MohiniResult<()> {
        if self.name_index.contains_key(&new_name) {
            return Err(MohiniError::AgentAlreadyExists(new_name));
        }
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> MohiniResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| MohiniError::AgentNotFound(id.to_string()))?;
