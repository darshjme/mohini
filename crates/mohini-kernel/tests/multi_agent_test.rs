...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p mohini-kernel --test multi_agent_test -- --nocapture
...
fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("mohini-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = MohiniKernel::boot_with_config(test_config()).expect("Kernel should boot");
...
"#,
    ...
    println!("\n{}", "=".repeat(60));
    println!("  MOHINI MULTI-AGENT FLEET TEST");
