...
impl KnowledgeStore {
    ...
    pub fn add_entity(&self, entity: Entity) -> MohiniResult<String> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn add_relation(&self, relation: Relation) -> MohiniResult<String> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| MohiniError::Internal(e.to_string()))?;
    ...
    pub fn query_graph(&self, pattern: GraphPattern) -> MohiniResult<Vec<GraphMatch>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| MohiniError::Internal(e.to_string()))?;
        ...
        for row_result in rows {
            let r = row_result.map_err(|e| MohiniError::Memory(e.to_string()))?;
