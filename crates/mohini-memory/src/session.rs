...
use chrono::Utc;
use mohini_types::agent::{AgentId, SessionId};
use mohini_types::error::{MohiniError, MohiniResult};
use mohini_types::message::{ContentBlock, Message, MessageContent, Role};
...
impl SessionStore {
    ...
    pub fn list_sessions(&self) -> MohiniResult<Vec<serde_json::Value>> {
        ...
        for row in rows {
            sessions.push(row.map_err(|e| MohiniError::Memory(e.to_string()))?);
        }
        Ok(sessions)
    }
...
impl SessionStore {
    ...
    pub fn load_canonical(&self, agent_id: AgentId) -> MohiniResult<CanonicalSession> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| MohiniError::Internal(e.to_string()))?;
            ...
            if to_compact > canonical.compaction_cursor {
                ...
                for msg in compacting {
                    let role = match msg.role {
                        mohini_types::message::Role::User => "User",
                        mohini_types::message::Role::Assistant => "Assistant",
                        mohini_types::message::Role::System => "System",
                    };
                    let text = msg.content.text_content();
                    if !text.is_empty() {
                        // Truncate individual messages in summary to keep it compact (UTF-8 safe)
                        let truncated = if text.len() > 200 {
                            format!("{}...", mohini_types::truncate_str(&text, 200))
                        } else {
                            text
                        };
    ...
    ) -> MohiniResult<(Option<String>, Vec<Message>)> {
    ...
    fn save_canonical(&self, canonical: &CanonicalSession) -> MohiniResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| MohiniError::Internal(e.to_string()))?;
                        ...
                        match block {
                            ...
                            ContentBlock::Thinking { thinking } => {
                                text_parts.push(format!(
                                    "[thinking: {}]",
                                    mohini_types::truncate_str(thinking, 200)
                                ));
                            }
...
mod tests {
    ...
    fn test_jsonl_mirror_write() {
        ...
        session
            .messages
            .push(mohini_types::message::Message::user("Hello"));
        session
            .messages
            .push(mohini_types::message::Message::assistant("Hi there!"));
        store.save_session(&session).unwrap();
