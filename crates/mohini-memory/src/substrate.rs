...
use mohini_types::agent::{AgentEntry, AgentId, SessionId};
use mohini_types::config::{VectorBackend, VectorStoreConfig};
use mohini_types::error::{MohiniError, MohiniResult};
use mohini_types::memory::{
...
impl MemorySubstrate {
    ...
    pub fn open(db_path: &Path, decay_rate: f32) -> MohiniResult<Self> {
        let conn = Connection::open(db_path).map_err(|e| MohiniError::Memory(e.to_string()))?;
        conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA busy_timeout=5000;")
            .map_err(|e| MohiniError::Memory(e.to_string()))?;
        run_migrations(&conn).map_err(|e| MohiniError::Memory(e.to_string()))?;
    ...
    pub fn open_in_memory(decay_rate: f32) -> MohiniResult<Self> {
        let conn =
            Connection::open_in_memory().map_err(|e| MohiniError::Memory(e.to_string()))?;
        run_migrations(&conn).map_err(|e| MohiniError::Memory(e.to_string()))?;
        let shared = Arc::new(Mutex::new(conn));
    ...
    /// Initialize an external vector store backend based on config.
    ///
    /// If the backend is `Qdrant`, attempts to connect and configure the store.
    /// On failure, logs a warning and continues with SQLite-only vector search.
    /// This method is async because external backends require network I/O.
    pub async fn init_vector_store(
        ...
        match config.backend {
            VectorBackend::Sqlite => {
                // Default — no external vector store needed
            }
            VectorBackend::Qdrant => {
                match QdrantVectorStore::new(config, agent_id).await {
                    Ok(store) => {
                        self.semantic.set_vector_store(Arc::new(store));
                        tracing::info!(
                            agent_id = agent_id,
                    ...
                    Err(e) => {
                        warn!(
                            error = %e,
                            agent_id = agent_id,
                            "Failed to initialize Qdrant vector store, falling back to SQLite"
                        );
                    }
                }
            }
    ...
    pub fn load_all_agents(&self) -> MohiniResult<Vec<AgentEntry>> {
        self.structured.load_all_agents()
    }

    /// List all saved agents.
    pub fn list_agents(&self) -> MohiniResult<Vec<(String, String, String)>> {
    ...
    pub fn structured_get(
        ...
        agent_id: AgentId,
        key: &str,
    ) -> MohiniResult<Option<serde_json::Value>> {
        self.structured.get(agent_id, key)
    }

    /// List all KV pairs for an agent.
    pub fn list_kv(&self, agent_id: AgentId) -> MohiniResult<Vec<(String, serde_json::Value)>> {
        self.structured.list_kv(agent_id)
    }

    /// Delete a KV entry for an agent.
    pub fn structured_delete(&self, agent_id: AgentId, key: &str) -> MohiniResult<()> {
    ...
    /// Get a session by ID.
    pub fn get_session(&self, session_id: SessionId) -> MohiniResult<Option<Session>> {
        self.sessions.get_session(session_id)
    }

    /// Save a session.
    pub fn save_session(&self, session: &Session) -> MohiniResult<()> {
        self.sessions.save_session(session)
    }

    /// Create a new empty session for an agent.
    pub fn create_session(&self, agent_id: AgentId) -> MohiniResult<Session> {
        self.sessions.create_session(agent_id)
    }

    /// List all sessions with metadata.
    pub fn list_sessions(&self) -> MohiniResult<Vec<serde_json::Value>> {
        self.sessions.list_sessions()
    }

    /// Delete a session by ID.
    pub fn delete_session(&self, session_id: SessionId) -> MohiniResult<()> {
        self.sessions.delete_session(session_id)
    }

    /// Delete all sessions belonging to an agent.
    pub fn delete_agent_sessions(&self, agent_id: AgentId) -> MohiniResult<()> {
        self.sessions.delete_agent_sessions(agent_id)
    }

    /// Delete the canonical (cross-channel) session for an agent.
    pub fn delete_canonical_session(&self, agent_id: AgentId) -> MohiniResult<()> {
    ...
    pub fn find_session_by_label(
        ...
        agent_id: AgentId,
        label: &str,
    ) -> MohiniResult<Option<Session>> {
        self.sessions.find_session_by_label(agent_id, label)
    }

    /// List all sessions for a specific agent.
    pub fn list_agent_sessions(&self, agent_id: AgentId) -> MohiniResult<Vec<serde_json::Value>> {
    ...
    ) -> MohiniResult<(Option<String>, Vec<mohini_types::message::Message>)> {
    ...
    pub fn store_llm_summary(
        ...
        agent_id: AgentId,
        summary: &str,
        kept_messages: Vec<mohini_types::message::Message>,
    ) -> MohiniResult<()> {
        self.sessions
    ...
    pub fn append_canonical(
        &self,
        agent_id: AgentId,
        messages: &[mohini_types::message::Message],
    ...
    pub fn load_paired_devices(&self) -> MohiniResult<Vec<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| MohiniError::Memory(e.to_string()))?;
        ...
        for row in rows {
            devices.push(row.map_err(|e| MohiniError::Memory(e.to_string()))?);
        }
        Ok(devices)
    }
    ...
    pub fn remove_paired_device(&self, device_id: &str) -> MohiniResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| MohiniError::Memory(e.to_string()))?;
    ...
    pub async fn task_post(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let db = conn.lock().map_err(|e| MohiniError::Internal(e.to_string()))?;
            db.execute(
    ...
    pub async fn task_claim(&self, agent_id: &str) -> MohiniResult<Option<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match result {
                ...
                Err(e) => Err(MohiniError::Memory(e.to_string())),
            }
        })
        .await
        .map_err(|e| MohiniError::Internal(e.to_string()))?
    ...
    pub async fn task_complete(&self, task_id: &str, result: &str) -> MohiniResult<()> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if rows == 0 {
                return Err(MohiniError::Internal(format!("Task not found: {task_id}")));
            }
            Ok(())
        })
        .await
    ...
    pub async fn task_list(&self, status: Option<&str>) -> MohiniResult<Vec<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = db.prepare(sql).map_err(|e| MohiniError::Memory(e.to_string()))?;
            ...
            for row in rows {
                tasks.push(row.map_err(|e| MohiniError::Memory(e.to_string()))?);
            }
            Ok(tasks)
        })
        .await
...
impl Memory for MemorySubstrate {
    async fn get(&self, agent_id: AgentId, key: &str) -> MohiniResult<Option<serde_json::Value>> {
    ...
    async fn delete(&self, agent_id: AgentId, key: &str) -> MohiniResult<()> {
        let store = self.structured.clone();
        let key = key.to_string();
    ...
    async fn forget(&self, id: MemoryId) -> MohiniResult<()> {
        let store = self.semantic.clone();
    ...
    async fn add_entity(&self, entity: Entity) -> MohiniResult<String> {
    ...
    async fn add_relation(&self, relation: Relation) -> MohiniResult<String> {
        let store = self.knowledge.clone();
    ...
    async fn query_graph(&self, pattern: GraphPattern) -> MohiniResult<Vec<GraphMatch>> {
        let store = self.knowledge.clone();
    ...
    async fn consolidate(&self) -> MohiniResult<ConsolidationReport> {
    ...
    async fn export(&self, format: ExportFormat) -> MohiniResult<Vec<u8>> {
        let _ = format;
        Ok(Vec::new())
    }

    async fn import(&self, _data: &[u8], _format: ExportFormat) -> MohiniResult<ImportReport> {
        Ok(ImportReport {
            entities_imported: 0,
