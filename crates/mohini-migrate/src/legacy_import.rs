//! LegacyImport workspace parser and migration engine.
//!
//! Real LegacyImport installations use a **single JSON5 config file** at
...
// ---------------------------------------------------------------------------
// LegacyImport JSON5 input types
// ---------------------------------------------------------------------------

/// Top-level openclaw.json structure.
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacySourceRoot {
    auth: Option<LegacyImportAuth>,
    models: Option<LegacyImportModels>,
    agents: Option<LegacyImportAgents>,
    tools: Option<LegacySourceRootTools>,
    channels: Option<LegacyImportChannels>,
    cron: Option<serde_json::Value>,
    hooks: Option<serde_json::Value>,
    skills: Option<LegacyImportSkills>,
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportAgents {
    defaults: Option<LegacyImportAgentDefaults>,
    list: Vec<LegacyImportAgentEntry>,
}

#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportAgentDefaults {
    model: Option<LegacyImportAgentModel>,
    workspace: Option<String>,
    tools: Option<LegacyImportAgentTools>,
...
#[serde(untagged)]
enum LegacyImportAgentModel {
    Simple(String),
    Detailed(LegacyImportAgentModelDetailed),
}

#[derive(Debug, Clone, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportAgentModelDetailed {
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportChannels {
    telegram: Option<LegacyImportTelegramConfig>,
    discord: Option<LegacyImportDiscordConfig>,
    slack: Option<LegacyImportSlackConfig>,
    whatsapp: Option<LegacyImportWhatsAppConfig>,
    signal: Option<LegacyImportSignalConfig>,
    matrix: Option<LegacyImportMatrixConfig>,
    #[serde(alias = "googlechat", alias = "googleChat")]
    google_chat: Option<LegacyImportGoogleChatConfig>,
    #[serde(alias = "msteams", alias = "msTeams")]
    teams: Option<LegacyImportTeamsConfig>,
    irc: Option<LegacyImportIrcConfig>,
    mattermost: Option<LegacyImportMattermostConfig>,
    feishu: Option<LegacyImportFeishuConfig>,
    imessage: Option<LegacyImportIMessageConfig>,
    bluebubbles: Option<LegacyImportBlueBubblesConfig>,
    #[serde(flatten)]
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportTelegramConfig {
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportDiscordConfig {
    token: Option<String>,
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportSlackConfig {
    bot_token: Option<String>,
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportWhatsAppConfig {
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportSignalConfig {
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportMatrixConfig {
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportGoogleChatConfig {
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportTeamsConfig {
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportMattermostConfig {
    bot_token: Option<String>,
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportFeishuConfig {
    app_id: Option<String>,
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportIMessageConfig {
...
#[derive(Debug, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
struct LegacyImportBlueBubblesConfig {
...
/// LegacyImport's legacy config.yaml structure.
...
/// LegacyImport's legacy agent.yaml structure.
#[derive(Debug, Deserialize)]
#[serde(default)]
...
/// LegacyImport's legacy channel config structure.
#[derive(Debug, Default, Deserialize)]
#[serde(default)]
...
// ---------------------------------------------------------------------------
// Mohini output types (TOML)
// ---------------------------------------------------------------------------

/// Mohini config.toml structure for serialization.
#[derive(Serialize)]
struct MohiniConfig {
    default_model: MohiniModelConfig,
    memory: MohiniMemorySection,
    network: MohiniNetworkSection,
    #[serde(skip_serializing_if = "Option::is_none")]
    channels: Option<toml::Value>,
}

#[derive(Serialize)]
struct MohiniModelConfig {
    provider: String,
    model: String,
    api_key_env: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Serialize)]
struct MohiniMemorySection {
    decay_rate: f32,
}

#[derive(Serialize)]
struct MohiniNetworkSection {
    listen_addr: String,
}
...
/// Map LegacyImport DM policy to Mohini DM policy string.
...
/// Map LegacyImport group policy to Mohini group policy string.
...
/// Split an LegacyImport model reference like `"provider/model"` into `(provider, model)`.
...
fn extract_primary_model(
    agent: &LegacyImportAgentEntry,
    defaults: Option<&LegacyImportAgentDefaults>,
) -> Option<String> {
    // Try agent-level model first
    if let Some(ref m) = agent.model {
        match m {
            LegacyImportAgentModel::Simple(s) => return Some(s.clone()),
            LegacyImportAgentModel::Detailed(d) => {
    ...
    if let Some(defs) = defaults {
        if let Some(ref m) = defs.model {
            match m {
                LegacyImportAgentModel::Simple(s) => return Some(s.clone()),
                LegacyImportAgentModel::Detailed(d) => return d.primary.clone(),
            }
        }
    }
    None
...
fn extract_fallback_models(
    ...
    // Try agent-level
    if let Some(LegacyImportAgentModel::Detailed(ref d)) = agent.model {
    ...
    // Fall back to defaults
    if let Some(defs) = defaults {
        if let Some(LegacyImportAgentModel::Detailed(ref d)) = defs.model {
            if !d.fallbacks.is_empty() {
                return d.fallbacks.clone();
            }
        }
...
fn find_config_file(dir: &Path) -> Option<PathBuf> {
    // Prefer JSON5 config (modern LegacyImport)
    for name in &[
        "openclaw.json",
        "mohini.json",
        "moldbot.json",
...
use mohini_types::tool_compat::{is_known_mohini_tool, map_tool_name};

/// Map LegacyImport tool profile to Mohini capability tool list.
...
/// Map LegacyImport provider name to Mohini provider name.
...
/// Map LegacyImport provider to its default API key env var.
...
/// Try to find the LegacyImport home directory.
pub fn detect_openclaw_home() -> Option<PathBuf> {
    ...
    let mut candidates: Vec<Option<PathBuf>> = vec![
        ...
        home.as_ref().map(|h| h.join(".mohini")),
    ...
    for candidate in candidates.into_iter().flatten() {
        if candidate.exists() && candidate.is_dir() {
            // Verify it looks like an LegacyImport workspace
...
/// Scan an LegacyImport workspace and return what's available for migration.
...
fn scan_from_json5(base: &Path, config_path: &Path, result: &mut ScanResult) {
    ...
    let root: LegacySourceRoot = match json5::from_str(&content) {
        Ok(r) => r,
        Err(_) => return,
    };
...
/// Result of scanning an LegacyImport workspace.
...
/// Run the LegacyImport migration.
pub fn migrate(options: &MigrateOptions) -> Result<MigrationReport, MigrateError> {
    ...
    info!("Migrating from LegacyImport: {}", source.display());

    let mut report = MigrationReport {
        source: "LegacyImport".to_string(),
...
// ---------------------------------------------------------------------------
// JSON5 migration flow (modern LegacyImport)
// ---------------------------------------------------------------------------
...
fn migrate_from_json5(
    ...
    let root: LegacySourceRoot = json5::from_str(&content)
...
fn migrate_config_from_json(
    ...
    let (provider, model) = root
        ...
        .and_then(|m| match m {
            LegacyImportAgentModel::Simple(s) => Some(s.clone()),
            LegacyImportAgentModel::Detailed(d) => d.primary.clone(),
        })
        .map(|m| split_model_ref(&m))
        .unwrap_or_else(|| {
            (
    ...
    let of_config = MohiniConfig {
        ...
        memory: MohiniMemorySection { decay_rate: 0.05 },
        network: MohiniNetworkSection {
    ...
    let config_content = format!(
        "# Mohini Agent OS configuration\n\
         # Migrated from LegacyImport on {}\n\n\
         {toml_str}",
...
fn migrate_channels_from_json(
    ...
    // --- BlueBubbles (skip — no Mohini adapter) ---
    if oc_channels.bluebubbles.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: "bluebubbles".to_string(),
            reason: "No Mohini adapter available — consider using the iMessage channel instead"
                .to_string(),
        });
    }
    ...
    for key in oc_channels.other.keys() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: key.clone(),
            reason: format!("Unknown channel '{key}' — not mapped to any Mohini adapter"),
        });
    }
            ...
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{id}': tool '{tool}' has no Mohini equivalent and was skipped"
                    ));
                }
...
fn convert_agent_from_json(
    ...
    let system_prompt = entry
        ...
        .unwrap_or_else(|| {
            format!(
                "You are {display_name}, an AI agent running on the Mohini Agent OS. You are helpful, concise, and accurate."
            )
        });
    ...
    toml_str.push_str(&format!(
        "# Mohini agent manifest\n# Migrated from LegacyImport agent '{id}'\n\n"
    ));
    toml_str.push_str(&format!(
        "name = \"{}\"\n",
    ...
    toml_str.push_str(&format!(
        "description = \"Migrated from LegacyImport agent '{id}'\"\n"
    ));
    toml_str.push_str("author = \"mohini\"\n");
...
fn resolve_default_tools(defaults: Option<&LegacyImportAgentDefaults>) -> Vec<String> {
    if let Some(defs) = defaults {
...
fn migrate_workspace_dirs(
    ...
    // LegacyImport stores workspaces in workspaces/<agent>/
...
fn report_skipped_features(root: &LegacySourceRoot, source: &Path, report: &mut MigrationReport) {
    // Cron jobs
    if root.cron.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Config,
            name: "cron".to_string(),
            reason: "Cron job scheduling not yet supported — use Mohini's ScheduleMode::Periodic instead".to_string(),
        });
    }
    ...
    if root.hooks.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Webhook hooks not supported — use Mohini's event system instead".to_string(),
        });
    }
        ...
        if let Some(ref entries) = skills.entries {
            if !entries.is_empty() {
                report.skipped.push(SkippedItem {
                    ...
                    reason: "Skills must be reinstalled via `mohini skill install`".to_string(),
                });
            }
        }
    }
    ...
    if source.join("memory-search").join("index.db").exists() {
        report.skipped.push(SkippedItem {
            ...
            reason: "SQLite vector index not portable — Mohini will rebuild embeddings"
                .to_string(),
        });
    }
    ...
    if root.session.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Session scope config differs — Mohini uses per-agent sessions by default"
                .to_string(),
        });
    }
    ...
    if root.memory.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason:
                "Memory backend config not migrated — Mohini uses SQLite with vector embeddings"
                    .to_string(),
        });
    }
}
...
fn migrate_legacy_config(
    ...
    if !config_path.exists() {
        report
            .warnings
            .push("No config.yaml found in LegacyImport workspace".to_string());
        return Ok(());
    }
        ...
        match *name {
            ...
            "bluebubbles" => {
                report.skipped.push(SkippedItem {
                    kind: ItemKind::Channel,
                    name: "bluebubbles".to_string(),
                    reason: "No Mohini adapter available — consider using the iMessage channel instead".to_string(),
                });
            }
            _ => {}
        }
    ...
    for entry in entries {
        ...
        match convert_legacy_agent(&agent_yaml, &agent_name) {
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{agent_name}': tool '{tool}' has no Mohini equivalent and was skipped"
                    ));
                }
...
fn convert_legacy_agent(
    ...
    let system_prompt = oc.system_prompt.unwrap_or_else(|| {
        format!(
            "You are {}, an AI agent running on the Mohini Agent OS. {}",
            oc.name,
    ...
    let mut toml_str = String::new();
    toml_str.push_str(&format!(
        "# Mohini agent manifest\n# Migrated from LegacyImport agent '{}'\n\n",
        oc.name
    ));
            ...
            for entry in entries.flatten() {
                ...
                if has_package_json && has_index {
                    report.skipped.push(SkippedItem {
                        kind: ItemKind::Skill,
                        name: name.clone(),
                        reason: "Node.js skill — run with `mohini skill install` after migration"
                            .to_string(),
                    });
                } else {
                    report.skipped.push(SkippedItem {
...
mod tests {
  ...
  session: { scope: "per-sender" }
    ...
    fn test_json5_full_migration() {
        ...
        let options = MigrateOptions {
            source: crate::MigrateSource::LegacyImport,
    ...
    fn test_json5_agent_model_parsing() {
        ...
        let model: LegacyImportAgentModel = serde_json::from_str(json_str).unwrap();
        match model {
            LegacyImportAgentModel::Detailed(d) => {
        ...
        let model: LegacyImportAgentModel = serde_json::from_str(json_str).unwrap();
        match model {
            LegacyImportAgentModel::Simple(s) => {
  ...
  channels: {
    ...
    slack: { botToken: "xoxb", appToken: "xapp" }
        ...
        let root: LegacySourceRoot = json5::from_str(json5_content).unwrap();
    ...
    fn test_json5_detect_home() {
        ...
        std::fs::write(dir2.path().join("mohini.json"), "{}").unwrap();
        let found = find_config_file(dir2.path());
        assert!(found.is_some());
        assert!(found.unwrap().ends_with("mohini.json"));
  ...
  agents: {
    ...
    #[test]
    fn test_is_known_mohini_tool() {
        assert!(is_known_mohini_tool("file_read"));
        assert!(is_known_mohini_tool("shell_exec"));
        assert!(is_known_mohini_tool("web_fetch"));
        assert!(!is_known_mohini_tool("Read"));
        assert!(!is_known_mohini_tool("unknown"));
    }
