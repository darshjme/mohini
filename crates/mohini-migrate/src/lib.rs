//! Migration engine for importing from other agent frameworks into Mohini.
//!
//! Supports importing agents, memory, sessions, skills, and channel configs
//! from LegacyImport and other frameworks.
...
impl std::fmt::Display for MigrateSource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::LegacyImport => write!(f, "LegacyImport"),
...
pub struct MigrateOptions {
    ...
    /// Path to the Mohini home directory.
...
pub fn run_migration(options: &MigrateOptions) -> Result<report::MigrationReport, MigrateError> {
    match options.source {
        MigrateSource::LegacyImport => legacy_import::migrate(options),
