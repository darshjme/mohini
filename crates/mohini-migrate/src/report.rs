...
impl MigrationReport {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        out.push_str("1. Review imported agent manifests in `~/.mohini/agents/`\n");
        out.push_str(
            "2. Review `~/.mohini/secrets.env` — verify tokens were migrated correctly\n",
        );
        out.push_str("3. Set any remaining API keys referenced in `~/.mohini/config.toml`\n");
        out.push_str("4. Start the daemon: `mohini start`\n");
        out.push_str("5. Test your agents: `mohini agent list`\n");
    ...
    pub fn print_summary(&self) {
        ...
        if !self.dry_run {
            println!("\n  Next steps:");
            println!("    mohini start");
            println!("    mohini agent list");
        }
    }
}
...
mod tests {
    ...
    fn test_empty_report() {
        ...
        assert!(md.contains("Migration Report: LegacyImport"));
    ...
    fn test_report_with_items() {
        let report = MigrationReport {
            ...
            imported: vec![MigrateItem {
                ...
                destination: "~/.mohini/agents/coder/agent.toml".to_string(),
            }],
