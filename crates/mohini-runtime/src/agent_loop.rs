...
use mohini_memory::session::Session;
use mohini_memory::MemorySubstrate;
use mohini_skills::registry::SkillRegistry;
use mohini_types::agent::AgentManifest;
use mohini_types::error::{MohiniError, MohiniResult};
use mohini_types::memory::{Memory, MemoryFilter, MemorySource};
use mohini_types::message::{
    ContentBlock, Message, MessageContent, Role, StopReason, TokenUsage,
};
use mohini_types::tool::{ToolCall, ToolDefinition};
...
pub struct AgentLoopResult {
    ...
    pub directives: mohini_types::message::ReplyDirectives,
}

/// Run the agent execution loop for a single user message.
///
/// This is the core of Mohini: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: mohini_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if text.trim() == "NO_REPLY" || parsed_directives.silent {
                    ...
                    return Ok(AgentLoopResult {
                        ...
                        cost_usd: None,
                        silent: true,
                        directives: mohini_types::message::ReplyDirectives {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: mohini_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in &response.tool_calls {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(MohiniError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: mohini_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: mohini_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    Err(MohiniError::MaxIterationsExceeded(max_iterations))
}
...
) -> MohiniResult<crate::llm_driver::CompletionResponse> {
    ...
    if let (Some(provider), Some(cooldown)) = (provider, cooldown) {
        match cooldown.check(provider) {
            CooldownVerdict::Reject {
                ...
                return Err(MohiniError::LlmDriver(format!(
    ...
    for attempt in 0..=MAX_RETRIES {
        match driver.complete(request.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(MohiniError::LlmDriver(user_msg));
            }
        }
    }

    Err(MohiniError::LlmDriver(
...
mod tests {
    ...
    use mohini_types::tool::ToolCall;
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = mohini_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = mohini_types::agent::AgentId::new();
        let mut session = mohini_memory::session::Session {
            id: mohini_types::agent::SessionId::new(),
            agent_id,
            messages: Vec::new(),
