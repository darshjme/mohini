...
use mohini_types::message::{ContentBlock, Message, StopReason, TokenUsage};
...
pub struct CompletionRequest {
    ...
    pub thinking: Option<mohini_types::config::ThinkingConfig>,
}
...
pub struct DriverConfig {
    ...
    /// `claude` CLI.  Defaults to `true` because Mohini runs as a daemon
    /// with no interactive terminal, so permission prompts would block
    /// indefinitely.  Mohini's own capability / RBAC layer already
