...
**⚠️ CRITICAL: Never review PRs in Mohini's own project folder!**
- Either use the project where the PR is submitted (if it's NOT ~/Projects/mohini)
- Or clone to a temp folder first

```bash
# Option 1: Review in the actual project (if NOT mohini)
bash workdir:~/Projects/some-other-repo background:true command:"codex review --base main"

# Option 2: Clone to temp folder for safe review (REQUIRED for mohini PRs!)
REVIEW_DIR=$(mktemp -d)
git clone https://github.com/mohini/mohini.git $REVIEW_DIR
...
**Why?** Checking out branches in the running Mohini repo can break the live instance!
...
8. **NEVER checkout branches in ~/Projects/mohini/** — that's the LIVE Mohini instance! Clone to /tmp or use git worktree for PR reviews
