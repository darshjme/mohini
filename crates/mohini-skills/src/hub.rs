//! SkillHub marketplace client — search and install skills from skillhub.ai.
//!
//! SkillHub hosts 3,000+ community skills in both SKILL.md (prompt-only)
//! and package.json (Node.js) formats. This client downloads, converts,
//! and security-scans skills before installation.
//!
//! API reference: <https://skillhub.ai/api/v1/>
...
// ---------------------------------------------------------------------------
// Retry constants for SkillHub API rate-limit handling
// ---------------------------------------------------------------------------

/// Maximum number of retry attempts for SkillHub API calls (including the first try).
...
// ---------------------------------------------------------------------------
// API response types (matching actual SkillHub v1 API — verified Feb 2026)
// ---------------------------------------------------------------------------
...
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SkillHubVersionInfo {
    #[serde(default)]
    pub version: String,
    #[serde(default)]
    pub created_at: i64,
...
pub struct SkillHubBrowseEntry {
    ...
    pub updated_at: i64,
    #[serde(default)]
    pub latest_version: Option<SkillHubVersionInfo>,
}

/// Paginated response from the browse endpoint.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SkillHubBrowseResponse {
    pub items: Vec<SkillHubBrowseEntry>,
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SkillHubSkillDetail {
    pub skill: SkillHubSkillInfo,
    #[serde(default)]
    pub latest_version: Option<SkillHubVersionInfo>,
    #[serde(default)]
    pub owner: Option<SkillHubOwner>,
...
pub type SkillHubListResponse = SkillHubBrowseResponse;
/// Alias kept for code that still references the old name.
pub type SkillHubSearchResults = SkillHubSearchResponse;
/// Alias kept for code that still references the old name.
pub type SkillHubEntry = SkillHubBrowseEntry;

/// Result of installing a skill from SkillHub.
#[derive(Debug, Clone)]
pub struct SkillHubInstallResult {
    /// Installed skill name.
    pub skill_name: String,
    /// Installed version.
    pub version: String,
    /// The skill slug on SkillHub.
    pub slug: String,
    /// Security warnings from the scan pipeline.
    pub warnings: Vec<SkillWarning>,
    /// Tool name translations applied (LegacyImport → Mohini).
    pub tool_translations: Vec<(String, String)>,
    /// Whether this is a prompt-only skill.
    pub is_prompt_only: bool,
}

/// Client for the SkillHub marketplace (skillhub.ai).
pub struct SkillHubClient {
    /// Base URL for the SkillHub API.
    base_url: String,
    /// HTTP client.
...
impl SkillHubClient {
    ...
    ///
    /// Uses the official SkillHub API at `https://skillhub.ai/api/v1`.
    pub fn new(cache_dir: PathBuf) -> Self {
        Self::with_url("https://skillhub.ai/api/v1", cache_dir)
    }

    /// Create a SkillHub client with a custom API URL.
        ...
        for attempt in 0..MAX_RETRIES {
            if attempt > 0 {
                ...
                debug!(
                    ...
                    delay_ms = total,
                    context,
                    "retrying SkillHub request after rate limit / server error"
                );
            ...
            match result {
                    ...
                    if status.as_u16() == 429 || status.is_server_error() {
                            ...
                            .and_then(|v| v.parse::<u64>().ok())
                        {
                            ...
                            if attempt + 1 < MAX_RETRIES {
                                debug!(
                                    retry_after_secs = ra,
                                    "SkillHub sent Retry-After, sleeping {capped}ms"
                                );
                            ...
                            if status.as_u16() == 429 {
                                return Err(SkillError::RateLimited(format!(
                                    "{context} returned 429 Too Many Requests after {MAX_RETRIES} attempts \
                                     — the SkillHub API rate limit has been exceeded, \
                ...
                Err(e) => {
                    ...
                    warn!(attempt, context, error = %e, "SkillHub request failed, will retry");
                }
            }
        }
    ...
    /// Search for skills on SkillHub using vector/semantic search.
    ///
    /// Uses `GET /api/v1/search?q=...&limit=...`.
    /// Returns `SkillHubSearchResponse` whose root key is `results` (not `items`).
    pub async fn search(
        &self,
        query: &str,
        limit: u32,
    ) -> Result<SkillHubSearchResponse, SkillError> {
        let url = format!(
            "{}/search?q={}&limit={}",
            self.base_url,
            urlencoded(query),
        ...
        let response = self.get_with_retry(&url, "SkillHub search").await?;

        let results: SkillHubSearchResponse = response
            .json()
            .await
            .map_err(|e| SkillError::Network(format!("Failed to parse SkillHub response: {e}")))?;
    ...
    pub async fn browse(
        ...
        limit: u32,
        cursor: Option<&str>,
    ) -> Result<SkillHubBrowseResponse, SkillError> {
        let mut url = format!(
            "{}/skills?limit={}&sort={}",
            self.base_url,
            limit.min(50),
        ...
        let response = self.get_with_retry(&url, "SkillHub browse").await?;

        let results: SkillHubBrowseResponse = response
            .json()
            .await
            .map_err(|e| SkillError::Network(format!("Failed to parse SkillHub browse: {e}")))?;
    ...
    pub async fn get_skill(&self, slug: &str) -> Result<SkillHubSkillDetail, SkillError> {
        let url = format!("{}/skills/{}", self.base_url, urlencoded(slug));

        let response = self.get_with_retry(&url, "SkillHub skill detail").await?;

        let detail: SkillHubSkillDetail = response
            .json()
            .await
            .map_err(|e| SkillError::Network(format!("Failed to parse SkillHub detail: {e}")))?;

        Ok(detail)
    }

    /// Helper: extract the version string from a browse entry.
    pub fn entry_version(entry: &SkillHubBrowseEntry) -> &str {
        entry
            .latest_version
            .as_ref()
    ...
    pub async fn get_file(&self, slug: &str, path: &str) -> Result<String, SkillError> {
        ...
        let response = self.get_with_retry(&url, "SkillHub file fetch").await?;

        let text = response
            .text()
            .await
            .map_err(|e| SkillError::Network(format!("Failed to read SkillHub file: {e}")))?;

        Ok(text)
    }

    /// Install a skill from SkillHub into the target directory.
    ///
    /// Security pipeline:
    /// 1. Download skill zip and compute SHA256
    /// 2. Detect format (SKILL.md vs package.json)
    /// 3. Convert to Mohini manifest
    ...
    pub async fn install(
        ...
        let manifest = if is_skillmd || legacy_compat::detect_skillmd(&skill_dir) {
            let converted = legacy_compat::convert_skillmd(&skill_dir)?;
            ...
            legacy_compat::write_prompt_context(&skill_dir, &converted.prompt_context)?;
            ...
            converted.manifest
        } else if legacy_compat::detect_legacy_import_skill(&skill_dir) {
            legacy_compat::convert_legacy_import_skill(&skill_dir)?
        } else {
        ...
        legacy_compat::write_mohini_manifest(&skill_dir, &manifest)?;

        let result = SkillHubInstallResult {
        ...
        info!(
            ...
            "Installed skill from SkillHub"
        );

        Ok(result)
    }

    /// Check if a SkillHub skill is already installed locally.
...
mod tests {
    ...
    fn test_browse_entry_serde_real_format() {
        // Matches actual SkillHub browse API response (verified Feb 2026)
        let json = r#"{
            "slug": "sonoscli",
    ...
    fn test_browse_response_serde() {
        ...
        let resp: SkillHubBrowseResponse = serde_json::from_str(json).unwrap();
    ...
    fn test_search_entry_serde_real_format() {
        // Matches actual SkillHub search API response (verified Feb 2026)
        let json = r#"{
    ...
    fn test_skill_detail_serde_real_format() {
        // Matches actual SkillHub detail API response (verified Feb 2026)
        let json = r##"{
            "skill": {
                "slug": "github",
                "displayName": "Github",
        ...
        let detail: SkillHubSkillDetail = serde_json::from_str(json).unwrap();
    ...
    #[test]
    fn test_skillhub_install_result() {
        let result = SkillHubInstallResult {
    ...
    #[test]
    fn test_skillhub_sort_str() {
        assert_eq!(SkillHubSort::Trending.as_str(), "trending");
        assert_eq!(SkillHubSort::Downloads.as_str(), "downloads");
        assert_eq!(SkillHubSort::Stars.as_str(), "stars");
    }

    #[test]
    fn test_skillhub_client_url() {
        let client = SkillHubClient::new(PathBuf::from("/tmp/cache"));
        assert_eq!(client.base_url, "https://skillhub.ai/api/v1");
    }

    #[test]
    fn test_entry_version_helper() {
        let entry = SkillHubBrowseEntry {
            slug: "test".to_string(),
            ...
            created_at: 0,
            updated_at: 0,
            latest_version: Some(SkillHubVersionInfo {
                version: "2.0.0".to_string(),
                created_at: 0,
                changelog: String::new(),
            }),
        };
        assert_eq!(SkillHubClient::entry_version(&entry), "2.0.0");
    }
}
