//! LegacyImport skill compatibility layer.
//!
//! LegacyImport skills come in two formats:
...
pub struct SkillMdMetadata {
    /// LegacyImport-specific metadata.
    #[serde(alias = "mohini", alias = "openclaw", alias = "clawdbot")]
    pub legacy_import: Option<LegacyImportMeta>,
}

/// LegacyImport-specific metadata in SKILL.md.
#[derive(Debug, Clone, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
pub struct LegacyImportMeta {
    ...
    /// System requirements.
    pub requires: Option<LegacyImportRequires>,
    /// Commands exposed by this skill.
    pub commands: Vec<LegacyImportCommand>,
}

/// System requirements declared by an LegacyImport skill.
#[derive(Debug, Clone, Default, Deserialize)]
#[serde(default)]
pub struct LegacyImportRequires {
...
/// A command declared by an LegacyImport skill.
#[derive(Debug, Clone, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
pub struct LegacyImportCommand {
    ...
    pub dispatch: Option<LegacyImportDispatch>,
}

/// Dispatch configuration for an LegacyImport command.
#[derive(Debug, Clone, Default, Deserialize)]
#[serde(default, rename_all = "camelCase")]
pub struct LegacyImportDispatch {
...
/// Result of converting a SKILL.md into Mohini format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (legacy_import_name → mohini_name).
...
/// Full conversion of a SKILL.md directory to Mohini format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    if let Some(ref meta) = frontmatter.metadata.legacy_import {
        ...
        // Convert commands to Mohini tool definitions
        for cmd in &meta.commands {
            if cmd.name.is_empty() {
                continue;
            }

            // Translate tool name if it's a known LegacyImport name
            let mohini_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_mohini_tool(&cmd.name) {
                cmd.name.clone()
            } else {
    ...
    let manifest = SkillManifest {
        skill: SkillMeta {
            ...
            author: String::new(),
            license: String::new(),
            tags: vec!["legacy_import-compat".to_string(), "prompt-only".to_string()],
        },
        ...
        source: Some(SkillSource::LegacyImport),
    };
...
/// Convert an in-memory SKILL.md string into Mohini format.
///
...
/// Check if a directory contains a valid LegacyImport Node.js skill.
pub fn detect_legacy_import_skill(dir: &Path) -> bool {
...
/// Convert an LegacyImport Node.js skill directory into an Mohini SkillManifest.
///
/// Reads package.json to extract name, version, description, and infers tool definitions.
pub fn convert_legacy_import_skill(dir: &Path) -> Result<SkillManifest, SkillError> {
    ...
    // Try to extract tool definitions from LegacyImport's skill metadata
    let tools = if let Some(legacy_import) = pkg.get("legacy_import") {
        extract_tools_from_legacy_import_meta(legacy_import)
    } else {
        vec![SkillToolDef {
            name: name.replace('-', "_"),
    ...
    info!("Converted LegacyImport skill: {name} ({} tools)", tools.len());
    ...
    Ok(SkillManifest {
        skill: SkillMeta {
            ...
            tags: vec!["legacy_import-compat".to_string()],
        },
        runtime: SkillRuntimeConfig {
...
/// Extract tool definitions from LegacyImport's package.json metadata.
fn extract_tools_from_legacy_import_meta(meta: &serde_json::Value) -> Vec<SkillToolDef> {
    let mut tools = Vec::new();
...
/// Write an Mohini skill.toml manifest for an LegacyImport skill.
pub fn write_mohini_manifest(dir: &Path, manifest: &SkillManifest) -> Result<(), SkillError> {
...
mod tests {
    ...
    #[test]
    fn test_detect_legacy_import_skill() {
        let dir = TempDir::new().unwrap();

        assert!(!detect_legacy_import_skill(dir.path()));
    ...
    #[test]
    fn test_convert_legacy_import_skill() {
        let dir = TempDir::new().unwrap();
        std::fs::write(
        ...
        let manifest = convert_legacy_import_skill(dir.path()).unwrap();
    ...
    #[test]
    fn test_convert_with_legacy_import_meta() {
        let dir = TempDir::new().unwrap();
        std::fs::write(
            dir.path().join("package.json"),
            r#"{
...
Use the gh CLI to manage PRs and issues."#;
        ...
        assert!(fm.metadata.legacy_import.is_some());
        let meta = fm.metadata.legacy_import.unwrap();
