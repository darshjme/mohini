//! Skill system for Mohini.
//!
//! Skills are pluggable tool bundles that extend agent capabilities.
//! They can be:
//! - TOML + Python scripts
//! - TOML + WASM modules
//! - TOML + Node.js modules (LegacyImport compatibility)
//! - Remote skills from SkillHub registry
...
pub enum SkillError {
    ...
    Network(String),
    #[error("Rate limited by SkillHub — please wait a moment and try again: {0}")]
    RateLimited(String),
...
pub enum SkillRuntime {
    ...
    Wasm,
    /// Node.js module (LegacyImport compatibility).
    Node,
...
pub enum SkillSource {
    /// Built into Mohini or manually installed.
    Native,
    /// Bundled at compile time (ships with Mohini binary).
    Bundled,
    /// Converted from LegacyImport format.
    #[serde(alias = "open_claw")]
    LegacyImport,
    /// Downloaded from SkillHub marketplace.
    #[serde(alias = "claw_hub")]
