...
impl SkillRegistry {
    ...
    /// Load all community skills (compiled into the binary).
    ///
    /// Called after `load_bundled()` but before `load_all()`.
    /// Community skills don't override bundled skills, but user-installed skills
    /// override both. Runs prompt injection scan on all community skills.
    pub fn load_community(&mut self) -> usize {
        let community = crate::community::types::all_community_skills();
        let mut count = 0;

        for skill in &community {
            // Don't override bundled skills
            ...
            // Defense in depth: scan community skill prompt content
            if let Some(ref ctx) = manifest.prompt_context {
                let warnings = SkillVerifier::scan_prompt_content(ctx);
                let has_critical = warnings.iter().any(|w| {
                    matches!(w.severity, crate::verify::WarningSeverity::Critical)
                ...
                if has_critical {
                    warn!(
                        skill = %manifest.skill.name,
                        "BLOCKED community skill: critical prompt injection patterns"
                    );
                    continue;
                }
            }
        ...
        if count > 0 {
            info!("Loaded {count} community skill(s)");
        }
        count
    }
    ...
    pub fn load_all(&mut self) -> Result<usize, SkillError> {
        ...
        for entry in entries.flatten() {
            ...
            if !manifest_path.exists() {
                ...
                if legacy_compat::detect_skillmd(&path) {
                    match legacy_compat::convert_skillmd(&path) {
                        Ok(converted) => {
                            // SECURITY: Scan prompt content for injection attacks
                            // before accepting the skill. 341 malicious skills were
                            // found on SkillHub — block critical threats at load time.
                            let warnings =
                            ...
                            info!(
                                skill = %converted.manifest.skill.name,
                                "Auto-converting SKILL.md to Mohini format"
                            );
                            if let Err(e) =
                                legacy_compat::write_mohini_manifest(&path, &converted.manifest)
                            {
                                warn!("Failed to write skill.toml for {}: {e}", path.display());
                                continue;
                            }
                            if let Err(e) = legacy_compat::write_prompt_context(
                                &path,
