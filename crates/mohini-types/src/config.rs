//! Configuration types for the Mohini kernel.
...
impl Default for WebhookTriggerConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            token_env: "MOHINI_WEBHOOK_TOKEN".to_string(),
...
pub struct DockerSandboxConfig {
    ...
    pub image: String,
    /// Container name prefix. Default: "mohini-sandbox".
...
impl Default for DockerSandboxConfig {
    fn default() -> Self {
        Self {
            ...
            container_prefix: "mohini-sandbox".to_string(),
...
pub struct VaultConfig {
    ...
    pub enabled: bool,
    /// Custom vault file path (default: ~/.mohini/vault.enc).
...
pub struct KernelConfig {
    /// Mohini home directory (default: ~/.mohini).
    pub home_dir: PathBuf,
    /// Data directory for databases (default: ~/.mohini/data).
    ...
    /// Whether to enable the MMP network layer.
    ...
    /// Root directory for agent workspaces. Default: `~/.mohini/workspaces`
    #[serde(default)]
...
/// Resolve the Mohini home directory.
///
/// Priority: `MOHINI_HOME` env var > `~/.mohini`.
fn mohini_home_dir() -> PathBuf {
...
pub struct MemoryConfig {
    ...
    /// Vector store configuration for external vector databases.
    #[serde(default)]
...
pub struct VectorStoreConfig {
    ...
    /// Prefix for collection names (e.g., "mohini_").
...
pub struct NetworkConfig {
    ...
    pub max_peers: u32,
    /// Pre-shared secret for MMP HMAC authentication (required when network is enabled).
...
pub struct MatrixConfig {
    ...
    /// Bot user ID (e.g., "@mohini:matrix.org").
    pub user_id: String,
...
pub struct IrcConfig {
    ...
    /// Channels to join (e.g., `["#mohini", "#general"]`).
    #[serde(default, deserialize_with = "deserialize_string_or_int_vec")]
...
mod tests {
    ...
    fn test_validate_missing_env_vars() {
        ...
        config.channels.discord = Some(DiscordConfig {
            bot_token_env: "MOHINI_TEST_NONEXISTENT_VAR_DC".to_string(),
            ..Default::default()
        });
        let warnings = config.validate();
