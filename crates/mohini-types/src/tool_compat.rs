//! Shared tool name mappings between LegacyImport and Mohini.
//!
//! These mappings are used by both the migration engine and the skill system
//! to normalize LegacyImport tool names into Mohini equivalents.

/// Map an LegacyImport tool name to its Mohini equivalent.
///
/// Returns `None` if the name has no known mapping (may already be
/// an Mohini tool name — check with [`is_known_mohini_tool`]).
pub fn map_tool_name(legacy_import_name: &str) -> Option<&'static str> {
    match legacy_import_name {
...
/// Normalize a tool name to its canonical Mohini form.
///
/// If the name is already a known Mohini tool, returns it as-is.
...
pub fn normalize_tool_name(name: &str) -> &str {
    if is_known_mohini_tool(name) {
        return name;
    }
    map_tool_name(name).unwrap_or(name)
}

/// Check if a tool name is a known Mohini built-in tool.
pub fn is_known_mohini_tool(name: &str) -> bool {
    matches!(
        name,
        "file_read"
            | "file_write"
...
mod tests {
    ...
    fn test_normalize_tool_name() {
        // Known Mohini tools pass through unchanged
    ...
    fn test_is_known_mohini_tool() {
        ...
        for tool in &known {
            assert!(is_known_mohini_tool(tool), "Expected {tool} to be known");
        }

        // Unknown
        assert!(!is_known_mohini_tool("unknown"));
        assert!(!is_known_mohini_tool("Read"));
        assert!(!is_known_mohini_tool("Bash"));
    }
}
