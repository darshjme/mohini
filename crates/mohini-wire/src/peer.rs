//! PeerNode — TCP server and client for the Mohini Wire Protocol.
//!
//! A [`PeerNode`] binds a local TCP listener and accepts incoming connections
//! from other Mohini kernels. It also connects outward to known peers. Each
...
/// SECURITY: Time-windowed nonce tracker to prevent MMP handshake replay attacks.
///
...
impl NonceTracker {
    ...
    pub fn check_and_record(&self, nonce: &str) -> Result<(), String> {
        ...
        if self.seen.contains_key(nonce) {
            return Err(format!("Nonce replay detected: {}", mohini_types::truncate_str(nonce, 16)));
        }
...
pub struct PeerConfig {
    ...
    /// Required — MMP refuses to start without it.
...
impl Default for PeerConfig {
    fn default() -> Self {
        Self {
            ...
            node_name: "mohini-node".to_string(),
...
mod tests {
    ...
    fn test_peer_config_default() {
        ...
        assert_eq!(config.node_name, "mohini-node");
