...
Mohini ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
mohini spawn orchestrator
mohini spawn coder
mohini spawn --template agents/writer/agent.toml
```
...
> General-purpose assistant. The default Mohini agent for everyday tasks, questions, and conversations.
...
```bash
mohini spawn meeting-assistant
...
mohini spawn coder --name "backend-coder"

# Spawn from a TOML file path
mohini spawn --template agents/custom/my-agent.toml

# List running agents
mohini agents

# Send a message
mohini message <agent-id> "Write a function to parse TOML files"

# Kill an agent
mohini kill <agent-id>
```
