# Mohini Configuration Reference

Complete reference for `config.toml`, covering every configurable field in the Mohini Agent OS.
...
Mohini reads its configuration from a single TOML file:
...
The simplest working configuration only needs an LLM provider API key set as an environment variable. With no config file at all, Mohini boots with Anthropic as the default provider:
...
```toml
# ============================================================
# Mohini Agent OS -- Complete Configuration Reference
# ============================================================

# --- Top-level fields ---
home_dir = "~/.mohini"             # Mohini home directory
data_dir = "~/.mohini/data"        # SQLite databases and data files
log_level = "info"                   # trace | debug | info | warn | error
api_listen = "127.0.0.1:50051"      # HTTP/WS API bind address
network_enabled = false              # Enable MMP peer-to-peer network
...
# --- Memory ---
[memory]
# sqlite_path = "~/.mohini/data/mohini.db"  # Auto-resolved if omitted
embedding_model = "all-MiniLM-L6-v2"
consolidation_threshold = 10000
decay_rate = 0.1

# --- Network (MMP Wire Protocol) ---
[network]
...
|-------|------|---------|-------------|
| `home_dir` | path | `~/.mohini` | Mohini home directory. Stores config, agents, skills. |
| `data_dir` | path | `~/.mohini/data` | Directory for SQLite databases and persistent data. |
...
| `network_enabled` | bool | `false` | Enable the MMP peer-to-peer network layer. |
...
```toml
[memory]
# sqlite_path = "/custom/path/mohini.db"
embedding_model = "all-MiniLM-L6-v2"
consolidation_threshold = 10000
decay_rate = 0.1
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sqlite_path` | path or null | `null` | Explicit path to the SQLite database file. When `null`, defaults to `{data_dir}/mohini.db`. |
...
Configures the MMP (Mohini Protocol) peer-to-peer networking layer with HMAC-SHA256 mutual authentication.
...
| `shared_secret` | string | `""` (empty) | Pre-shared secret for MMP HMAC-SHA256 mutual authentication. **Required** when `network_enabled = true`. Both sides must use the same secret. Redacted in logs. |
...
| `user_id` | string | `""` | Bot user ID (e.g., `"@mohini:matrix.org"`). |
...
| `nick` | string | `"mohini"` | Bot nickname. |
| `password_env` | string or null | `null` | Env var holding the server password (optional). |
| `channels` | list of strings | `[]` | IRC channels to join (e.g., `["#mohini", "#general"]`). |
...
| `nick` | string | `"mohini"` | Bot nickname in Twitch chat. |
| `default_agent` | string or null | `null` | Agent name to route messages to. |
...
| `username` | string | `"mohini"` | Bot username in Mumble. |
...
Agent-to-Agent protocol configuration, enabling inter-agent communication across Mohini instances.
