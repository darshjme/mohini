...
cargo tauri signer generate -w ~/.tauri/mohini.key
```
...
Your private key was saved to: ~/.tauri/mohini.key
```
...
Open `crates/mohini-desktop/tauri.conf.json` and replace:
...
|---|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | Contents of `~/.tauri/mohini.key` | Yes |
...
The following icon files must exist in `crates/mohini-desktop/icons/`:
...
## 5. Set Up the `mohini.sh` Domain

**Status:** BLOCKING for install scripts — users run `curl -sSf https://mohini.sh | sh`.
...
- `https://mohini.sh/install.ps1` → serves `scripts/install.ps1`
...
```bash
docker build -t mohini:local .
docker run --rm mohini:local --version
docker run --rm -p 4200:4200 -v mohini-data:/data mohini:local start
```
...
- Migration path from LegacyImport
...
grep '"version"' crates/mohini-desktop/tauri.conf.json
...
4. Open the v0.1.0 app — after 10 seconds it should:
   - Show "Mohini Updating..." notification
...
mohini --version  # Should print v0.1.0
