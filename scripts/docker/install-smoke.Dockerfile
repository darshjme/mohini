...
ARG MOHINI_SMOKE_FULL=0
RUN if [ "$MOHINI_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$MOHINI_SMOKE_FULL" = "1" ] && [ -f "$HOME/.mohini/bin/mohini" ]; then \
        $HOME/.mohini/bin/mohini --version && \
        echo "PASS: mohini binary works"; \
    else \
