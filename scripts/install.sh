#!/usr/bin/env bash
# Mohini installer — works on Linux, macOS, WSL
# Usage: curl -sSf https://mohini.sh | sh
#
# Environment variables:
#   MOHINI_INSTALL_DIR  — custom install directory (default: ~/.mohini/bin)
#   MOHINI_VERSION      — install a specific version tag (default: latest)
...
detect_platform() {
    ...
    case "$OS" in
        ...
        mingw*|msys*|cygwin*)
            ...
            echo "    irm https://mohini.sh/install.ps1 | iex"
            echo ""
            echo "  Or download the .msi installer from:"
            echo "    https://github.com/$REPO/releases/latest"
            echo ""
            echo "  Or install via cargo:"
            echo "    cargo install --git https://github.com/$REPO mohini-cli"
            exit 1
            ;;
...
install() {
    ...
    echo ""
    echo "  Mohini Installer"
    echo "  =================="
    echo ""

    # Get latest version
    if [ -n "${MOHINI_VERSION:-}" ]; then
        VERSION="$MOHINI_VERSION"
    ...
    URL="https://github.com/$REPO/releases/download/$VERSION/mohini-$PLATFORM.tar.gz"
    CHECKSUM_URL="$URL.sha256"

    echo "  Installing Mohini $VERSION for $PLATFORM..."
    mkdir -p "$INSTALL_DIR"

    # Download to temp
    TMPDIR=$(mktemp -d)
    ARCHIVE="$TMPDIR/mohini.tar.gz"
    ...
    if [ "$OS" = "darwin" ]; then
        if command -v xattr &>/dev/null; then
            xattr -cr "$INSTALL_DIR/mohini" 2>/dev/null || true
        fi
        if command -v codesign &>/dev/null; then
            if ! codesign --force --sign - "$INSTALL_DIR/mohini"; then
                echo ""
                echo "  Warning: ad-hoc code signing failed."
                echo "  On Apple Silicon, the binary may be killed (SIGKILL) by Gatekeeper."
                echo "  Try manually: xattr -cr $INSTALL_DIR/mohini && codesign --force --sign - $INSTALL_DIR/mohini"
                echo ""
            fi
        fi
    fi
    ...
    if [ -n "$SHELL_RC" ] && ! grep -q "mohini" "$SHELL_RC" 2>/dev/null; then
    ...
    # Verify installation
    if "$INSTALL_DIR/mohini" --version >/dev/null 2>&1; then
        INSTALLED_VERSION=$("$INSTALL_DIR/mohini" --version 2>/dev/null || echo "$VERSION")
        echo ""
        echo "  Mohini installed successfully! ($INSTALLED_VERSION)"
    else
        echo ""
        echo "  Mohini binary installed to $INSTALL_DIR/mohini"
    fi
