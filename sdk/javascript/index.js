/**
 * @mohini/sdk — Official JavaScript client for the Mohini Agent OS REST API.
 *
 * Usage:
 *   const { Mohini } = require("@mohini/sdk");
...
class MohiniError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "MohiniError";
    this.status = status;
    this.body = body;
  }
}
...
class Mohini {
  /**
   * @param {string} baseUrl - Mohini server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new MohiniError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new MohiniError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { Mohini: Mohini, MohiniError: MohiniError };
